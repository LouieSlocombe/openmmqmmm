...
class DFTBTheory():
    def __init__(self, dftbdir=None, hamiltonian="XTB", xtb_method="GFN2-xTB", printlevel=2, label="DFTB",
        ...
        if hamiltonian != "XTB":
            ...
            if slaterkoster_dict is None:
                print(
                    "Error: No dictionary of Slater-Koster files provided (slaterkoster_dict keyword). This is necessary")
                ashexit()
            hamiltonian = "DFTB"
        ...
        if maxmom_dict is None:
            ...
            print("Check this!")
            self.maxmom_dict = {'H': 's', 'B': 'p', 'C': 'p', 'N': 'p', 'O': 'p', 'F': 'p', 'Ca': 'p', 'K': 'p',
                                'Mg': 'p', 'Na': 'p',
        ...
        if hubbard_derivs_dict is None:
            ...
            if self.ThirdOrderFull is True or self.ThirdOrder is True:
                print(
                    "Error: For DFTB3 (third-order term) calculations a dictionary of atomic Hubbard derivatives must be provided")
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        # Write inputfile
        write_DFTB_input(self.hamiltonian, self.xtb_method, xyzfilename + '.xyz', qm_elems, current_coords, charge,
                         mult, PC=PC, Grad=Grad,
                         slaterkoster_dict=self.slaterkoster_dict, maxmom_dict=self.maxmom_dict, MMcharges=MMcharges,
                         MMcoords=current_MM_coords,
                         Gauss_blur_width=self.Gauss_blur_width, SCC=self.SCC, ThirdOrderFull=self.ThirdOrderFull,
                         ThirdOrder=self.ThirdOrder,
        ...
        # Grab energy
        if PC is True:
            self.energy, self.gradient, self.pcgradient = grab_energy_gradient_DFTB("detailed.out", len(current_coords),
                                                                                    Grad=Grad, PC=True,
                                                                                    numpc=len(MMcharges))
        else:
            self.energy, self.gradient, self.pcgradient = grab_energy_gradient_DFTB("detailed.out", len(current_coords),
...
#
def write_DFTB_input(hamiltonian, xtbmethod, xyzfilename, elems, coords, charge, mult, PC=False, MMcharges=None,
                     MMcoords=None, Grad=False, SCC=True,
                     slaterkoster_dict=None, maxmom_dict=None, Gauss_blur_width=0.0, ThirdOrderFull=False,
                     ThirdOrder=False,
