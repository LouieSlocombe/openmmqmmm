...
class NWChemTheory:
    def __init__(self, nwchemdir=None, filename='nwchem', openshell=False, printlevel=2, label="NWChem",
        ...
        if nwchemdir == None:
            print(BC.WARNING,
                  f"No nwchemdir argument passed to {self.theorynamelabel}Theory. Attempting to find nwchemdir variable inside settings_ash",
                  BC.END)
            try:
...
def run_NWChem(nwchemdir, filename, numcores=1):
    with open(filename + '.out', 'w') as ofile:
        if numcores > 1:
            process = sp.run(['mpirun', '-np', str(numcores), nwchemdir + '/nwchem', filename + '.nw'], check=True,
                             stdout=ofile, stderr=ofile, universal_newlines=True)
        else:
            process = sp.run([nwchemdir + '/nwchem', filename + '.nw'], check=True, stdout=ofile, stderr=ofile,
