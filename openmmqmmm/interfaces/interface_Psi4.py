...
class Psi4Theory:
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, label=None, mm_elems=None,
            elems=None, Grad=False, PC=False, numcores=None, pe=False, potfile='', restart=False, charge=None,
            mult=None):
        ...
        if self.runmode == 'library':
            ...
            try:
                ...
                print(BC.FAIL,
                      "Problem importing psi4. Make sure psi4 has been installed as part of same Python as ASH", BC.END)
            ...
            if restart == True:
                ...
                print("Renaming lastrestart.180 to {}".format(
                    os.path.splitext(self.filename + '.out')[0] + '.default.' + PID + '.180.npy'))
                os.rename('lastrestart.180',
                          os.path.splitext(self.filename + '.out')[0] + '.default.' + PID + '.180.npy')
            else:
            ...
            PID = str(os.getpid())
            try:
                print("Renaming {} to lastrestart.180".format(
                    os.path.splitext(self.filename + '.out')[0] + '.default.' + PID + '.180.npy'))
                os.rename(os.path.splitext(self.filename + '.out')[0] + '.default.' + PID + '.180.npy',
                          'lastrestart.180')
            except:
                pass
        ...
        # Psithon INPUT-FILE BASED INTERFACE. Creates Psi4 inputfiles and runs Psithon as subprocessses
            ...
            with open(self.filename + '.inp', 'w') as inputfile:
                ...
                if PC == True:
                    ...
                    for mmcharge, mmcoord in zip(MMcharges, current_MM_coords):
                        inputfile.write(
                            'Chrgfield.extern.addCharge({}, {}, {}, {})\n'.format(mmcharge, mmcoord[0], mmcoord[1],
                                                                                  mmcoord[2]))
            ...
            with open(self.filename + '.out', 'w') as ofile:
                ...
                process = sp.run(['psi4', '-m', '-i', self.filename + '.inp', '-o', self.filename + '.out', '-n',
                                  '{}'.format(str(numcores))], check=True, stdout=ofile, stderr=ofile,
