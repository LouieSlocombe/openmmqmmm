...
MRCC_basis_dict = {'DZ': 'cc-pVDZ', 'TZ': 'cc-pVTZ', 'QZ': 'cc-pVQZ', '5Z': 'cc-pV5Z', 'ADZ': 'aug-cc-pVDZ',
                   'ATZ': 'aug-cc-pVTZ', 'AQZ': 'aug-cc-pVQZ',
                   'A5Z': 'aug-cc-pV5Z'}
...
class MRCCTheory:
    def __init__(self, mrccdir=None, filename='mrcc', printlevel=2,
        ...
        if mrccdir is None:
            print(BC.WARNING,
                  "No mrccdir argument passed to MRCCTheory. Attempting to find mrccdir variable inside settings_ash",
                  BC.END)
            try:
        ...
        if self.no_basis_read_orbs is True:
            if 'basis' in mrccinput:
                print(
                    "Error: basis keyword found in mrccinput. should not be used when no_basis_read_orbs option is active")
                ashexit()

        if self.keep_orientation is True:
            print(
                "Warning: keep_orientation options is on (by default)! This means that the original input structure in an MRCC job is kept and symmetry is turned off")
            print(
                "This is necessary for gradient calculations and also is you want the density for the original structure")
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        if Grad:
            write_mrcc_input(self.mrccinput, charge, mult, qm_elems, current_coords, numcores, Grad=True,
                             ...
                             frozen_core_option=self.frozencore_string, no_basis_read_orbs=self.no_basis_read_orbs)
...
def run_mrcc(mrccdir, filename, parallelization, numcores):
    ...
    with open(filename, 'w') as ofile:
        ...
        if parallelization == 'OMP':
            ...
            process = sp.run([mrccdir + '/dmrcc'], env=os.environ, check=True, stdout=ofile, stderr=ofile,
...
def write_mrcc_input(mrccinput, charge, mult, elems, coords, numcores, Grad=False, keep_orientation=False,
                     PC_coords=None, PC_charges=None,
    ...
    with open("MINP", 'w') as inpfile:
        ...
        if no_basis_read_orbs is True:
            print(
                "Warning: no_basis_read_orbs is True. Adding iface=cfour option, means that integrals will be read from fort.55 and fort.56 files")
            inpfile.write(
                "iface=cfour")  # Activates CFour interface (just means that MRCC will read in fort.55 and fort.56 files)
        ...
        if keep_orientation is True:
            print(
                "keep_orientation is True. Turning off symmetry and doing dummy QM/MM calculation to preserve orientation")
...
def run_MRCC_HLC_correction(coords=None, elems=None, fragment=None, charge=None, mult=None, theory=None, method='CCSDT',
                            basis='TZ',
...
def convert_MRCC_Molden_file(mrccoutputfile=None, moldenfile=None, mrccdensityfile=None, multiwfndir=None,
                             printlevel=2):
    print("convert_MRCC_Molden_file")

    if multiwfndir == None:
        print(BC.WARNING,
              "No multiwfndir argument passed to multiwfn_run. Attempting to find multiwfndir variable inside settings_ash",
              BC.END)
        try:
    ...
    write_multiwfn_input_option(option="mrcc-density", frozenorbitals=frozen_orbs, densityfile=mrccdensityfile,
