...
class MLatomTheory(Theory):
    def __init__(self, printlevel=2, numcores=1, label="mlatom",
        ...
        try:
            ...
            print(
                "You probably also have to do: pip install joblib, scipy torch torchani tqdm matplotlib statsmodels h5py pyh5md")
            print()
        ...
        if self.method is not None:
            if 'AIQM1' in self.method:
                ...
                print(
                    f"Either an invalid  {self.method} or unknown method (to MLatomTheory interface) was selected. Exiting.")
                ashexit()
        ...
        if self.ml_model is not None:
            ...
            if ml_model.lower() == 'kreg':
                ...
                if ml_program is None:
                    print(
                        "ml_program keyword was not set and is required for KREG. Options are: 'KREG_API' and 'MLatomF'. Exiting.")
                ...
                if self.multi_model_run:
                    ...
                    for mfile in model_file:
                        self.models.append(
                            ml.models.kreg(model_file=mfile, ml_program=ml_program, nthreads=self.numcores))
                else:
                ...
                if shutil.which('dp'):
                    ...
                    print(
                        "See DeePMD-kit website: https://docs.deepmodeling.com/projects/deepmd/en/stable/install/easy-install.html")
    ...
    def setup_sparrow(self):
        ...
        print(
            "Also make sure dftd4 (https://github.com/dftd4/dftd4) is in your environment. Possible installation  via:  conda install dftd4")
    ...
    def train(self, molDB_xyzfile=None, molDB_scalarproperty_file=None,
            ...
            if len(hyperparameters) > 0:
                ...
                if 'sigma' in hyperparameters:
                    self.model.hyperparameters[
                        'sigma'].minval = 2 ** -5  # modify the default lower bound of the hyperparameter sigma
                self.model.optimize_hyperparameters(subtraining_molecular_database=subtrainDB,
                                                    ...
                                                    training_kwargs={'property_to_learn': property_to_learn,
                                                                     'prior': 'mean'},
        ...
        # Analyzing
        if molDB_xyzvecproperty_file is not None:
            self.result_molDB = analyzing(molDB, ref_value='energy', est_value='estimated_y',
                                          ref_grad='energy_gradients', est_grad='estimated_xyz_derivatives_y',
                                          set_name="molDB")
            self.result_subtrainDB = analyzing(subtrainDB, ref_value='energy', est_value='estimated_y',
                                               ref_grad='energy_gradients', est_grad='estimated_xyz_derivatives_y',
                                               set_name="subtrainDB")
            self.result_valDB = analyzing(valDB, ref_value='energy', est_value='estimated_y',
                                          ref_grad='energy_gradients', est_grad='estimated_xyz_derivatives_y',
                                          set_name="valDB")
        else:
        ...
        print(
            "Statistics saved as attributes:  result_molDB, result_subtrainDB and result_valDB of MLatomTheory object")
        print()
        ...
        if self.multi_model_run:
            ...
            if PC is True:
                ...
                if self.qm_program == "xtb":
                    ...
                    print(
                        "PC-embedded QM/MM calculations are right now only possible if qm_program is xtb (e.g. AIQM2)")
                    ashexit()
            model.predict(molecule=molecule, calculate_energy=True,
                          calculate_energy_gradients=Grad,
                          calculate_hessian=False)
