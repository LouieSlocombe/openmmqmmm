...
from ash.modules.module_coords import write_xyzfile, cubic_box_size, bounding_box_dimensions
...
# 'XTB'
class CP2KTheory:
    def __init__(self, cp2kdir=None, cp2k_bin_name=None, filename='cp2k', printlevel=2, basis_dict=None,
                 potential_dict=None, label="CP2K",
                 periodic=False, periodic_type='XYZ', qm_periodic_type=None, xtb_periodic=False, cell_dimensions=None,
                 cell_vectors=None,
        ...
        if self.numcores != 1:
            ...
            if parallelization == 'Mixed':
                if self.mixed_mpi_procs is None or self.mixed_omp_threads is None:
                    print(
                        "Error: mixed_mpi_procs and mixed_omp_threads keywords must be provided for Mixed parallelization")
                    ashexit()
                if 'psmp' not in self.cp2k_bin_name:
                    print("Error: Mixed parallelization only possible with cp2k.psmp")
                    ashexit()
                print("Mixed parallelization requested. Will use", self.mixed_mpi_procs, "MPI processes and",
                      self.mixed_omp_threads, "OpenMP threads")
                ...
                if 'ssmp' not in self.cp2k_bin_name:
                    if 'psmp' not in self.cp2k_bin_name:
                        ...
                        ashexit()
                print(
                    f"OpenMP parallelization requested. 1 CP2K process will be launched with {numcores} OpenMP threads")
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        if PC is True:
            ...
            write_CP2K_input(method='QMMM', jobname='ash', center_coords=self.center_coords, qm_elems=qm_elems,
                             ...
                             OT=self.OT, OT_minimizer=self.OT_minimizer, OT_preconditioner=self.OT_preconditioner,
                             OT_linesearch=self.OT_linesearch,
                             outer_SCF=self.outer_SCF, outer_SCF_optimizer=self.outer_SCF_optimizer,
                             OT_energy_gap=self.OT_energy_gap)
        else:
            # No QM/MM
            # QM-CELL
            ...
            write_CP2K_input(method=self.method, jobname='ash', center_coords=self.center_coords, qm_elems=qm_elems,
                             ...
                             Grad=Grad, filename='cp2k', charge=charge, mult=mult,
                             coordfile=system_xyzfile, scf_convergence=self.scf_convergence,
        ...
        if self.parallelization == 'Mixed':
            run_CP2K(self.cp2kdir, self.cp2k_bin_name, self.filename, numcores=self.numcores,
                     paramethod=self.parallelization,
        ...
        if Grad is True:
            ...
            if PC is True:
                self.pcgradient = grab_pcgradient_CP2K(f'ash-{self.filename}-1_0.xyz', len(MMcharges),
                                                       len(current_coords))
    ...
    with open(filename + '.out', 'w') as ofile:
        if numcores > 1:
            ...
            if paramethod == 'MPI':
                ...
                process = sp.run(["mpirun", "--bind-to", "none", f"-np", f"{str(numcores)}", f"{cp2kdir}/{bin_name}",
                                  f"{filename}.inp"], check=True, stdout=ofile, stderr=ofile, universal_newlines=True)
            elif paramethod == 'OMP':
                # OpenMP-parallel, no-MPI
                print(f"Launching OpenMP parallel CP2K using {numcores} OpenMP threads")
                os.environ["OMP_NUM_THREADS"] = str(numcores)
                process = sp.run([cp2kdir + f'/{bin_name}', filename + '.inp'], check=True, stdout=ofile, stderr=ofile,
                ...
                print(
                    f"Will use {mixed_mpi_procs} MPI processes and {mixed_omp_threads} OpenMP threads per process for a total of {mixed_mpi_procs * mixed_omp_threads} cores")
                if mixed_mpi_procs * mixed_omp_threads != numcores:
                    print(
                        f"Error: mixed_mpi_procs*mixed_omp_threads={mixed_mpi_procs}*{mixed_omp_threads} is not equal to numcores={numcores}")
                    ashexit()
                os.environ["OMP_NUM_THREADS"] = str(mixed_omp_threads)
                process = sp.run(
                    ["mpirun", "--bind-to", "none", f"-np", f"{str(mixed_mpi_procs)}", f"{cp2kdir}/{bin_name}",
                     f"{filename}.inp"], check=True, stdout=ofile, stderr=ofile, universal_newlines=True)
            else:
...
def write_CP2K_input(method='QUICKSTEP', jobname='ash-CP2K', center_coords=True, qm_elems=None,
                     ...
                     qm_cell_dims=None, qm_periodic_type=None, xtb_periodic=False, basis_file='BASIS',
...
def find_cp2k(cp2kdir, cp2k_bin_name):
    ...
    if cp2kdir == None:
        # No cp2kdir
        print(BC.WARNING,
              f"No cp2kdir argument passed to CP2KTheory. Attempting to find cp2kdir variable inside settings_ash",
              BC.END)
        try:
            ...
            if cp2k_bin_name == None:
                print(
                    f"cp2k_bin_name variable not set. Will search for multiple binaries in PATH, using this order: {cp2k_binaries}")
            else:
