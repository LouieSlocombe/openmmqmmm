...
class MNDOTheory:
    def __init__(self, mndodir=None, filename='mndo', method=None, printlevel=2, label="MNDO",
        ...
        if mndodir == None:
            print(BC.WARNING,
                  f"No mndodir argument passed to {self.theorynamelabel}Theory. Attempting to find mndodir variable inside settings_ash",
                  BC.END)
            try:
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        # Grab energy
        if PC is True:
            self.energy, self.gradient, self.pcgradient = grab_energy_gradient_mndo(f"{self.filename}.out",
                                                                                    len(current_coords), Grad=Grad,
                                                                                    PC=True, numpc=len(MMcharges))
        else:
            self.energy, self.gradient, self.pcgradient = grab_energy_gradient_mndo(f"{self.filename}.out",
                                                                                    len(current_coords), Grad=Grad,
                                                                                    PC=False)
...
def write_mndo_input(method, filename, elems, coords, charge, mult, PC=False, MMcharges=None, MMcoords=None, Grad=False,
                     restart=True, diis=False, guess_option=0, scfconv=6):
    # init_time=time.time()
    ...
    if PC is True:
        ...
        ash.functions.functions_general.fast_nparray_write(pcdata, float_format="%-12.7f %-12.7f %-12.7f %-8.4f",
                                                           filename=f"{filename}.inp", writemode="a")
