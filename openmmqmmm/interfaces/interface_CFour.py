...
CFour_basis_dict = {'DZ': 'PVDZ', 'TZ': 'PVTZ', 'QZ': 'PVQZ', '5Z': 'PV5Z', 'ADZ': 'AUG-PVDZ', 'ATZ': 'AUG-PVTZ',
                    'AQZ': 'AUG-PVQZ',
                    'A5Z': 'AUG-PV5Z'}
...
class CFourTheory:
    def __init__(self, cfourdir=None, printlevel=2, cfouroptions=None, numcores=1,
        ...
        if self.CALC == 'CCSDT' or self.CALC == 'CCSDTQ' or self.CALC == 'CCSDT(Q)':
            if self.ABCDTYPE == 'AOBASIS':
                print(
                    "Warning: ABCDTYPE=AOBASIS not possible for higher-order CC (CCSDT and beyond). Changing to ABCDTYPE=STANDARD")
        ...
        if self.basis == 'SPECIAL':
            if specialbasis != None:
                ...
                print(
                    "basis option is: SPECIAL (default) but no specialbasis dictionary provided. Please provide this (specialbasis keyword).")
                ashexit()
        else:
            self.specialbasis = []
        ...
        if ash_basisfile != None:
            # ash_basisfile
            print("Copying ASH basis-file {} from {} to current directory".format(ash_basisfile,
                                                                                  ash.settings_ash.ashpath + '/databases/basis-sets/cfour/'))
    ...
    def cfour_call(self):
        ...
        with open(self.filename + '.out', 'w') as ofile:
            if self.parallelization == 'MKL':
                ...
                process = sp.run([f"{self.cfourdir}/xcfour"], env=os.environ, check=True, stdout=ofile, stderr=ofile,
    ...
    def cleanup(self):
        ...
        files = ["THETA", "NTOTAL", "MOINTS", "NATMOS", "DIPOL", "FILES", "IIII", "JMOLplot", "MOABCD", "JOBARC",
                 "OPTARC", "NEWMOS", "EFG", "BASINFO.DATA", "VPOUT", "OLDMOS", "NEWFOCK", "MOL",
        ...
        if PC is True:
            ...
            with open("pcharges", "w") as pfile:
                ...
                for mmcharge, mmcoord in zip(MMcharges, current_MM_coords):
                    pfile.write(
                        f"{ash.constants.ang2bohr * mmcoord[0]} {ash.constants.ang2bohr * mmcoord[1]} {ash.constants.ang2bohr * mmcoord[2]} {mmcharge}\n")
...
def run_CFour_HLC_correction(coords=None, elems=None, charge=None, mult=None, fragment=None, theory=None,
                             method='CCSDT',
