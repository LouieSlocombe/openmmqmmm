...
from ash.functions.functions_general import ashexit, BC, check_program_location
...
def create_adaptive_minimal_basis_set(directory=None, fragment=None, xyzfile=None, bin_name="qvSZP",
    ...
    if basisfile_path is None or ecpfile_path is None:
        print(
            "Error: you must provide a basisfile_path and ecpfile_path (download from https://github.com/grimme-lab/qvSZP/)")
        ashexit()

    print("Running qvSZP. Will write output to qvSZP.out")
    with open('qvSZP.out', 'w') as ofile:

        args_list = [directory + f'/{bin_name}', '--struc', xyzfile, '--chrg', str(charge), '--bfile', basisfile_path,
                     '--efile', ecpfile_path]
