...
from ash.functions.functions_general import ashexit, natural_sort, print_line_with_mainheader, check_program_location
...
def call_plumed_sum_hills(hillsfile=None, path_to_plumed=None, ndim=None, binsg=[99, 99], ming=[0, 0], maxg=[10, 10]):
    ...
    if ndim == None:
        ...
        print(
            f"Calling plumed sum_hills with  grid settings: --bin {binsg[0]},{binsg[1]} --min {ming[0]},{ming[1]} --max {maxg[0]},{maxg[1]}")
        os.system(
            f'plumed sum_hills --hills HILLS --bin {binsg[0]},{binsg[1]} --min {ming[0]},{ming[1]} --max {maxg[0]},{maxg[1]}')


# Sumhills and plot
def plumed_MTD_plot_1CV(CV1_type=None, CV1_unit_label=None, imageformat='png', dpi=200, path_to_plumed=None,
                        hillsfile=None):
    path_to_plumed = check_program_location(path_to_plumed, 'path_to_plumed', 'plumed')

    call_plumed_sum_hills(hillsfile=hillsfile, path_to_plumed=path_to_plumed, ndim=1, binsg=[99, 99], ming=[0, 0],
                          maxg=[10, 10])
...
def plumed_FES_plot_1CV(fesfile="fes.dat", imageformat='png', dpi=200, CV1_type=None, CV1_unit_label=None):
    try:
        ...
        print(
            "Problem importing matplotlib (make sure it is installed in your environment). Plotting is not possible but continuing.")
...
def plumed_MTD_analyze(path_to_plumed=None, Plot_To_Screen=False, CV1_type=None, CV2_type=None, temperature=None,
                       CV1_indices=None, CV2_indices=None, plumed_length_unit=None, plumed_energy_unit="kj/mol",
                       plumed_time_unit=None,
    ...
    if MultipleWalker == True:
        ...
        if CV1_grid_limits is None:
            ...
            if CVnum == 1:
                call_plumed_sum_hills(path_to_plumed, 'HILLS.ALL', ndim=CVnum, ming=[CV1_grid_limits[0] / CV1_scaling],
                                      maxg=[CV1_grid_limits[1] / CV1_scaling])
            elif CVnum == 2:
                # Changing input unit from Angstrom to nm or degree to radian
                call_plumed_sum_hills(path_to_plumed, 'HILLS.ALL', ndim=CVnum,
                                      ming=[CV1_grid_limits[0] / CV1_scaling, CV2_grid_limits[0] / CV2_scaling],
        ...
        if CV1_grid_limits == None:
            ...
            # Changing input unit from Angstrom to nm or degree to radian
            if CVnum == 1:
                call_plumed_sum_hills(path_to_plumed, 'HILLS', ndim=CVnum, ming=[CV1_grid_limits[0] / CV1_scaling],
                                      maxg=[CV1_grid_limits[1] / CV1_scaling])
            elif CVnum == 2:
                # Changing input unit from Angstrom to nm or degree to radian
                call_plumed_sum_hills(path_to_plumed, 'HILLS', ndim=CVnum,
                                      ming=[CV1_grid_limits[0] / CV1_scaling, CV2_grid_limits[0] / CV2_scaling],
                                      maxg=[CV1_grid_limits[1] / CV1_scaling, CV2_grid_limits[1] / CV2_scaling])
        HILLSFILELIST = ['HILLS']
        # Single COLVAR file
    ...
    if CVnum == 1:
        ...
        plt.plot(final_rc, Relfreeenergy_kcal, marker='o', linestyle='-', linewidth=1, markersize=3,
                 label='G({} K)'.format(temperature))
        if PotCurve == True:
            plt.plot(potcurve_degs, potcurve_Relenergy_kcal, marker='o', linestyle='-', markersize=3, linewidth=1,
                     label='E(0 K)', color='orange')
        ...
        colorscatter = plt.scatter(final_rc, final_rc2, c=Relfreeenergy_kcal, marker='o', linestyle='-', linewidth=1,
                                   cmap=cm)
        ...
        colorscatter = plt.scatter(finalcolvar_value_list_flat, finalcolvar2_value_list_flat, c=time_flat, marker='o',
                                   s=2, linestyle='-', linewidth=1, cmap=cm)
        cbar = plt.colorbar(colorscatter)
        ...
        plt.ylabel('{} ({})'.format(CV2_type, CV2_indices), fontsize='small')
        cm = plt.cm.get_cmap(colormap)
        colorscatter2 = plt.scatter(finalcolvar_value_list_flat, finalcolvar2_value_list_flat,
                                    c=biaspot_value_kcal_list_flat, marker='o', linestyle='-', linewidth=1, cmap=cm)
...
class plumed_ASH():
    def __init__(self, path_to_plumed_kernel=None, bias_type="MTD", fragment=None, CV1_type=None, CV1_indices=None,
                 ...
                 temperature=300.0, hills_file="HILLS", colvar_file="COLVAR", height=0.01243, sigma1=None, sigma2=None,
                 biasfactor=6.0, timestep=None,
        ...
        if dynamics_program == "ASE":
            print(
                "Dynamics program ASE is recognized. Setting Plumed units to Angstrom (distance), eV (energy) and ps (time).")
            ...
            print(
                "Dynamics program OpenMM is recognized. Setting Plumed units to nm (distance), kJ/mol (energy) and ps (time).")
            print("sigma and height values should reflect this. ")
        ...
        self.plumedobj.cmd("readInputLine",
                           "UNITS LENGTH={} ENERGY={} TIME={}".format(self.plumed_length_unit, self.plumed_energy_unit,
                                                                      self.plumed_time_unit))
        ...
        if bias_type == "MTD":
            # 1D metadynamics
            CV1_indices_string = ','.join(map(str, [i + 1 for i in
                                                    CV1_indices]))  # Change from 0 to 1 based indexing and converting to text-string
            ...
            if CV2_type != None:
                ...
                self.plumedobj.cmd("readInputLine",
                                   "cv2: {} ATOMS={}".format(self.CV2_type.upper(), CV2_indices_string))
                CV_string = "cv1,cv2"
                sigma_string = str(sigma1) + "," + str(sigma2)

            self.plumedobj.cmd("readInputLine",
                               "METAD LABEL=MTD ARG={} PACE={} HEIGHT={} SIGMA={} FILE={} BIASFACTOR={} TEMP={}".format(
                                   CV_string, pace_num,
            ...
            self.plumedobj.cmd("readInputLine",
                               "PRINT STRIDE={} ARG={},MTD.bias FILE={}".format(stride_num, CV_string, colvar_file))
        else:
