...
class ipieTheory:
    def __init__(self, pyscftheoryobject=None, filename='input.json', printlevel=2, label="ipie",
        ...
        try:
            ...
            print(
                "Problem finding executables: ipie and 'pyscf_to_ipie.py' scripts in PATH. Did you install correctly? ")
            ashexit()
        try:
            import pyblock
        ...
        if self.frozencore is True:
            ...
            if self.mcscf is True:
                ...
                sp.call([self.pyscf_to_ipie_exe, '-i', self.checkpointfilename, '-j', self.filename, '--mcscf',
                         '--frozen-core', str(self.frozen_core_orbs)])
            else:
                sp.call([self.pyscf_to_ipie_exe, '-i', self.checkpointfilename, '-j', self.filename, '--frozen-core',
                         str(self.frozen_core_orbs)])
        else:
            if self.mcscf is True:
                print("MCSCF option true.")
                sp.call([self.pyscf_to_ipie_exe, '-i', self.checkpointfilename, '-j', self.filename, '--mcscf'])
