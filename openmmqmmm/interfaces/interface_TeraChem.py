...
class TeraChemTheory:
    def __init__(self, terachemdir=None, filename='terachem', printlevel=2, label="TeraChem",
        ...
        if terachemdir == None:
            print(BC.WARNING,
                  f"No terachemdir argument passed to {self.theorynamelabel}Theory. Attempting to find terachemdir variable inside settings_ash",
                  BC.END)
            try:
        ...
        if Grad:
            if PC is True:
                write_terachem_input(self.teracheminput, charge, mult, qm_elems, current_coords,
                                     Grad=True, PCfile=self.filename + '.pc', xyzfilename=self.filename + '.xyz',
...
def run_terachem(terachemdir, filename):
    with open(filename + '.out', 'w') as ofile:
        process = sp.run([terachemdir + '/terachem', filename + '.in'], check=True, stdout=ofile, stderr=ofile,
