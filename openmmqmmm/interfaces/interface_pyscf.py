...
class PySCFTheory:
    def __init__(self, printsetting=False, printlevel=2, numcores=1, label="pyscf", platform='CPU', GPU_pcgrad=False,
                 mf_object=None,
                 scf_type=None, basis=None, basis_file=None, cartesian_basis=None, ecp=None, functional=None,
                 gridlevel=5, symmetry=False,
                 ...
                 fractional_occupation=False, scf_maxiter=50, scf_noiter=False, direct_scf=True, GHF_complex=False,
                 collinear_option='mcol',
        ...
        if mf_object is None:
            ...
            if basis is None and basis_file is None and fcidumpfile is None:
                print(
                    "Error: You must either provide a basis or a basis_file keyword . Basis can a name (string) or dict (elements as keys)")
                print(
                    "basis_file should be a string of the filename containing basis set for each element, in NWChem format")
            ...
            if functional is not None:
                ...
                if "dm21" in self.functional.lower():
                    ...
                    try:
                        ...
                        print(
                            "Make sure DM21 is installed in the Python environment. See https://github.com/google-deepmind/deepmind-research/tree/master/density_functional_approximation_dm21")
                        ashexit()
            else:
        ...
        if CASSCF is True and CAS is False:
            ...
            if CASSCF_mults != None:
                if type(CASSCF_numstates) == int:
                    print(
                        "For a state-averaged CASSCF with different spin multiplicities, CASSCF_numstates must be a list")
                    print(
                        "Example: if CASSCF_mults=[1,3] you should set CASSCF_numstates=[2,4] for 2 singlet and 4 triplet states")
                    ashexit()
        ...
        if self.CAS is True:
            ...
            if self.AVAS is True or self.DMET_CAS is True:
                ...
                if self.CAS_AO_labels is None:
                    print(
                        "AVAS/DMET_CAS requires CAS_AO_labels keyword. Specify as e.g. CAS_AO_labels=['Fe 3d', 'Fe 4d', 'C 2pz']")
                    ashexit()
            if self.APC is True:
    ...
    def determine_frozen_core(self, elems):
        ...
        FC_elems = {'H': 0, 'He': 0, 'Li': 0, 'Be': 0, 'B': 2, 'C': 2, 'N': 2, 'O': 2, 'F': 2, 'Ne': 2,
                    ...
                    'K': 10, 'Ca': 10, 'Sc': 10, 'Ti': 10, 'V': 10, 'Cr': 10, 'Mn': 10, 'Fe': 10, 'Co': 10, 'Ni': 10,
                    'Cu': 10, 'Zn': 10,
        ...
        if chkfile != None:
            ...
            if chkfile_scftype == "UHF":
                ...
                if num_occ != self.num_basis_functions:
                    print(
                        f"Number of occupations ({num_occ}) in chkfile does not match number of basis functions ({self.num_basis_functions})")
                ...
                if self.num_electrons != int(sum_occ):
                    if self.printlevel >= 1:
                        print(
                            f"Number of electrons in checkpointfile ({sum_occ}) does not match number of electrons in molecule ({self.num_electrons})")
                        print("Ignoring MOs in chkfile and continuing")
                    return False
            else:
                # RESTRICTED
    ...
    def setup_guess(self):
        ...
        if self.dm is not None:
            ...
            if self.scf_type == 'RHF' or self.scf_type == 'RKS' or self.scf_type == 'GHF' or self.scf_type == 'GKS':
                if self.dm.shape[0] != self.num_basis_functions:
                    print(
                        f"Warning: The density matrix shape {self.dm.shape} does not match number of basis functions ({self.num_basis_functions}).")
                ...
                if self.dm[0].shape[0] != self.num_basis_functions:
                    if self.dm[0].shape[0] * 2 != self.num_basis_functions:
                        print(
                            f"Warning: The density matrix shape {self.dm[0].shape[0]} does not match number of basis functions ({self.num_basis_functions}).")
                        print("This density matrix can not be correct. Ignoring")
                        self.dm = None
                    return None
            return self.dm
        ...
        # NOTHING SPECIFIED: so possible AUTOSTART
            ...
            if self.guess not in ['minao', 'atom', 'huckel', 'vsap', '1e']:
                print("Guess option not recognized. Valid guess options are: ",
                      ['minao', 'atom', 'huckel', 'vsap', '1e'])
                ashexit()
    ...
    def calc_losc(self):
        ...
        self.write_orbitals_to_Moldenfile(self.mol, self.mf.mo_coeff, self.mf.mo_occ, self.mf.mo_energy,
                                          label="CAN-orbs")
        ...
        if self.LOSC_method == 'postSCF':
            ...
            a, b, losc_data = pyscf_losc.post_scf_losc(losc_func,
                                                       self.mf, return_losc_data=True, orbital_energy_unit='eV',
                                                       window=self.LOSC_window)
            ...
            self.write_orbitals_to_Moldenfile(self.mol, self.orbitalets, self.mf.mo_occ, self.orbitalets_energies,
                                              label="LOSC-orbs")
            ...
            print("loscmf:", loscmf)
            self.loscmf = loscmf
            self.write_orbitals_to_Moldenfile(self.mol, self.loscmf.mo_coeff, self.loscmf.mo_occ, self.loscmf.mo_energy,
                                              label="LOSC-SCF-orbs")
    ...
    def run_CAS(self, elems=None):
        ...
        if self.AVAS is True:
            ...
            print("This means that SCF-orbitals are ignored and we will read MO coefficients from chkfile:",
                  self.moreadfile)
            ...
            if self.cas_nmin != None and self.cas_nmax != None:
                print(
                    f"Active space will be determined from MP2 natorbs. NO threshold parameters: cas_nmin={self.cas_nmin} and cas_nmax={self.cas_nmax}")
                ...
                print(
                    f"To get this same active space in another calculation you can also do: active_space=[{nel_cas},{norb_cas}]")
            else:
            ...
            if type(mo_coefficients) is np.ndarray:
                ...
                if mo_coefficients.ndim == 2:
                    if isinstance(mf, pyscf.scf.rohf.ROHF):  # Works for ROKS too
                        print(
                            "Warning: Meanfield object is ROHF, but not supported by RCCSD. PySCF will convert to UHF and use UCCSD")
                        ...
                        print(
                            "Warning: single set of MO coefficients found but UHF/UKS determinant used. Duplicating MO coefficients")
                        mo_coefficients = [mo_coefficients, mo_coefficients]
        else:
    ...
    def set_embedding_options(self, PC=False, MM_coords=None, MMcharges=None):
        ...
        if PC is True:
            ...
            if self.platform == 'GPU':
                ...
                mm_mol = mm_mole.create_mm_mol(MM_coords, self.PBC_lattice_vectors, MMcharges, radii=self.radii,
    ...
    def run_SCF(self, mf=None, dm=None, max_cycle=None):
        ...
        print_time_rel(module_init_time, modulename='pySCF run_SCF', moduleindex=2, currprintlevel=self.printlevel,
                       currthreshold=2)
        return scf_result
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        # Actual run
        return self.actualrun(current_coords=current_coords, current_MM_coords=current_MM_coords, MMcharges=MMcharges,
                              qm_elems=qm_elems,
                              elems=elems, Grad=Grad, PC=PC, numcores=numcores, pe=pe, potfile=potfile, restart=restart,
                              label=label,
    ...
    def prepare_run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None,
        ...
        try:
            ...
            if os.environ['OMP_NUM_THREADS'] != '1':
                print(
                    "Warning: Environment variable OMP_NUM_THREADS should be set to 1. PySCF may not run properly in parallel")
        except:
            pass
        try:
            ...
            if os.environ['MKL_NUM_THREADS'] != '1':
                print(
                    "Warning: Environment variable MKL_NUM_THREADS should be set to 1. PySCF may not run properly in parallel")
        except:
            pass
    ...
    def actualrun(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None,
        ...
        if self.postSCF is True:
            ...
            if self.MP2 is True:
                print("MP2 is on !")
                MP2_energy, mp2object = self.run_MP2(frozen_orbital_indices=self.frozen_orbital_indices,
                                                     MP2_DF=self.MP2_DF)
                self.energy = MP2_energy

                if self.MP2_density is True:
                    self.run_MP2_density(mp2object, MP2_DF=self.MP2_DF,
                                         DFMP2_density_relaxed=self.DFMP2_density_relaxed)
            #####################
            # COUPLED CLUSTER
            #####################
            if self.CC is True:
                ...
                self.CC_energy, self.ccobject = self.run_CC(self.mf, frozen_orbital_indices=self.frozen_orbital_indices,
                                                            CCmethod=self.CCmethod,
        ...
        if Grad:
            ...
            if PC is True:
                ...
                checkpoint = time.time()
                self.pcgrad = pyscf_pointcharge_gradient(self.mol, np.array(current_MM_coords_bohr),
                                                         np.array(MMcharges), dm, GPU=self.GPU_pcgrad)
...
def pyscf_MR_correction(fragment, theory=None, MLmethod='CCSD(T)'):
    ...
    act_list = list(range(theory.firstMO_index,
                          theory.lastMO_index + 1))  # The range that Dice-SHCI used. Generalize this to DMRGTheory also ?
    ...
    CC_energy, CC_object = theory.pyscftheoryobject.run_CC(theory.pyscftheoryobject.mf,
                                                           frozen_orbital_indices=frozen_orbital_indices,
                                                           CCmethod=MLmethod,
...
def pyscf_CCSD_T_natorb_selection(fragment=None, pyscftheoryobject=None, numcores=1, thresholds=[1.997, 0.02],
    ...
    if pyscftheoryobject.MP2 is True or pyscftheoryobject.CC is True:
        print(
            "Error: pySCFTHeory object already has MP2 or CC turned on. This is not allowed for pyscf_natorb_CCSD_T_selection")
        print(
            "The pySCFTheory object should only contain settings for an SCF mean-field object (basis set, scf_type, functional etc.)")
        ashexit()
    ...
    MP2_natocc, MP2_natorb, mp2_dm = pyscfcalc.run_MP2_density(mp2object, MP2_DF=MP2_DF,
                                                               DFMP2_density_relaxed=DFMP2_density_relaxed)
    ...
    result = ASH_Results(label="pyscf_CCSD_T_natorb_selection", energy=CC_energy, charge=fragment.charge,
                         mult=fragment.mult)
...
""")
    ...
    ca, cb = pyscftheoryobj.mf.mo_coeff[:, :pyscftheoryobj.mol.nelec[0]], pyscftheoryobj.mf.mo_coeff[
        :, :pyscftheoryobj.mol.nelec[1]]
...
def DFA_error_analysis(fragment=None, DFA_obj=None, REF_obj=None, DFA_DM=None, REF_DM=None, REF_E=None, DFA_E=None,
    ...
    mo_occ, mo_energy, mo_coeff, ref_DM_inv = KS_inversion_kspies(REF_obj, REF_DM, method=inversion_method,
                                                                  ...
                                                                  ZMP_lambda=ZMP_lambda, ZMP_levelshift=ZMP_levelshift,
                                                                  ZMP_cycles=ZMP_cycles, DF=DF)
