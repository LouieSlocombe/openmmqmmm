...
class GaussianTheory:
    def __init__(self, gaussiandir=None, gauss_executable='g16', filename='gaussian', file_extension='.com',
                 printlevel=2, label="Gaussian",
        ...
        if gaussiandir is None:
            print(BC.WARNING,
                  f"No gaussiandir argument passed to {self.theorynamelabel}Theory. Attempting to find gaussiandir variable inside settings_ash",
                  BC.END)
            try:
        ...
        if 'opt' in gaussianinput.lower():
            print(BC.FAIL, f"Error. You should not input an optimization keyword in gaussianinput-string. Exiting...",
                  BC.END)
            ashexit()
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        write_Gaussian_input(self.gaussianinput, charge, mult, qm_elems, current_coords, memory=self.memory,
                             ...
                             Grad=Grad, PC=PC, filename=self.filename, file_extension=self.file_extension,
                             numcores=self.numcores)
        ...
        if Grad is True:
            ...
            if PC is True:
                print(
                    "A gradient calculation with PCs has been requested. Unfortunately, point charge gradients are not available")
                ashexit()
...
def run_Gaussian(gaussiandir, gauss_exe=None, file_extension=None, filename='gaussian'):
    ...
    with open(filename + '.out', 'w') as ofile:
        process = sp.run([gaussiandir + f'/{gauss_exe}', filename + file_extension], check=True, stdout=ofile,
                         stderr=ofile, universal_newlines=True)


def write_Gaussian_input(gaussianinput, charge, mult, elems, coords, filename='gaussian', file_extension=None,
                         memory='800MB', numcores=1,
