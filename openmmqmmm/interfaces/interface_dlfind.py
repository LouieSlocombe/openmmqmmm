...
from ash.modules.module_coords import check_charge_mult, print_internal_coordinate_table, \
    write_xyzfile, elemstonuccharges
...
def DLFIND_optimizer(jobtype=None, theory=None, fragment=None, fragment2=None, charge=None, mult=None,
    ...
    if theory is None or fragment is None:
        ...
        ashexit()
    optimizer = DLFIND_optimizerClass(jobtype=jobtype, theory=theory, fragment=fragment, fragment2=fragment2,
...
class DLFIND_optimizerClass:
    ...
    def __init__(self, jobtype=None, fragment=None, fragment2=None, theory=None, charge=None, mult=None,
                 maxcycle=250, tolerance=4.5E-4, tolerance_e=1E-6,
                 printlevel=2, result_write_to_disk=True, actatoms=None, frozenatoms=None, residues=None,
                 constraints=None,
        ...
        if jobtype is None and icoord is None:
            print(
                "Error: You must either select a jobtype keyword (e.g. opt, neb, dimer, instanton) or select DL-FIND icoord and iopt codes")
            ...
            print(
                "For other coordinate-systems: choose icoord=0 (cartesian), icoord=2 (hdlc-tc), icoord=3 (dlc-prim), icoord=3 (dlc-tc)")
            print(
                "For other opt algorithms: choose iopt codes: 0: sd, 1: cg-autorestart, 2: cg-restart10, 3: lbfgs, 10: P-RFO")
            icoord = 1
            iopt = 3
        ...
        def ash_e_g_func(coordinates, iimage, kiter, theory):
            ...
            energy, gradient = theory.run(current_coords=coordinates_ang, elems=self.fragment.elems, charge=charge,
                                          mult=mult, Grad=True)
        ...
        def store_results(a, nvar, switch, energy, coordinates, iam):
            if switch > 0:
                ...
                if self.icoord >= 100 and self.icoord < 150 and switch == -1:
                    ...
                    with open("DLFIND_NEBpath_all.xyz", "a") as trajfile:
                        for imageid in list(range(1, self.nimage)):
                            ...
                            for el, cord in zip(self.fragment.elems, self.NEB_geometries[imageid]):
                                trajfile.write(
                                    el + "  " + str(cord[0]) + " " + str(cord[1]) + " " + str(cord[2]) + "\n")
                    ...
                    if nimage in self.NEB_geometries:
                        ...
                        write_xyzfile(fragment.elems, self.NEB_geometries[nimage], "DLFIND_CIgeo_traj", printlevel=2,
                                      writemode='a', title=f"Energy: {self.NEB_energies_dict[nimage]}")
                return
            # Write traj to disk
            ...
            if self.icoord < 100:
                ...
                write_xyzfile(fragment.elems, coordinates_ang, "DLFIND_opt_traj", printlevel=2, writemode='a',
            ...
            # Traj-writing for dimer
                ...
                if switch == 1:
                    # 1: actual geometry
                    write_xyzfile(fragment.elems, coordinates_ang, "DLFIND_dimertraj_1", printlevel=2, writemode='a',
                                  title=f"Energy: {energy}")
                    self.current_geo = coordinates_ang
                elif switch == 2:
                    # Approximate:
                    ...
                    # transition mode
                    write_xyzfile(fragment.elems, coordinates_ang, "DLFIND_dimertraj_2", printlevel=2, writemode='a',
                                  title=f"Energy: {energy}")
                elif switch == 3:
                    write_xyzfile(fragment.elems, coordinates_ang, "DLFIND_dimertraj_3", printlevel=2, writemode='a',
                                  title=f"Energy: {energy}")
        ...
        remove_files = ['DLFIND_opt_traj.xyz', 'DLFIND_dimertraj_1.xyz', 'DLFIND_dimertraj_2.xyz',
                        'DLFIND_dimertraj_3.xyz', 'DLFIND_NEBpath_current.xyz', 'DLFIND_NEBpath_all.xyz',
                        'DLFIND_CIgeo_traj.xyz']
