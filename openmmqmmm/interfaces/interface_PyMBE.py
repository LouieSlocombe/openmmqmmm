...
class PyMBETheory:
    def __init__(self, pymbedir=None, filename='pymbe_', printlevel=2, label="PyMBE",
        ...
        if pymbedir == None:
            print(BC.WARNING,
                  "No pymbedir argument passed to PyMBETheory. Attempting to find pymbedir variable inside settings_ash",
                  BC.END)
            try:
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        for totE, corrE, label in zip(mbe_results_dict["tot_energies"], mbe_results_dict["corr_energies"],
                                      mbe_results_dict["labels"]):
...
def run_pymbe(pymbedir, filename, numcores=1):
    with open(filename, 'w') as ofile:
        if numcores == 1:
            ...
            process = sp.run([pymbedir + '/src/main.py'], check=True, stdout=ofile, stderr=ofile,
                             universal_newlines=True)
        else:
            print(f"Running PyMBE using {numcores} cores using external MPI4PY mpiexec command")
            process = sp.run(['mpiexec', '-np', str(numcores), pymbedir + '/src/main.py'], check=True, stdout=ofile,
