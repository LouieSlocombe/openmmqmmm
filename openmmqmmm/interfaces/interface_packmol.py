...
def packmol_solvate(packmoldir=None, ligand_files=None, num_mols_ligands=None, solvent_files=None, solvents_ratio=None,
                    tolerance=2.0, result_file="final", shape="box",
                    min_coordinates=[0.0, 0.0, 0.0], max_coordinates=[40.0, 40.0, 40.0], total_density=None,
                    sphere_center=None, sphere_radius=None):
    ligands_mass = []
    solvents_mass = []
    ...
    if solvent_files is None:
        ...
        if total_density is None and solvents_ratio is None:
            print(
                "Error: When providing solvent_files, you must specify either total_density or solvents_ratio (as molecule counts)!")
            ashexit()

    # --- Density Calculation ---
    if total_density is not None:
        print(
            f"A density of {total_density} g/ml was specified. Calculating number of molecules based on density and volume.")
        if ligand_files:
        ...
        volume = 0.0
        if shape.lower() == "box":
            volume = (max_coordinates[0] - min_coordinates[0]) * (max_coordinates[1] - min_coordinates[1]) * (
        ...
        print(
            "Will compute mass of ligands and solvents and calculate number of molecules for each solvent based on ratio and total density.")
...
"""
    ...
    if solvent_files and result_filename.endswith(".pdb"):
        ...
        print(
            "Will now try to use ASH function writepdb_with_connectivity to write a new PDB-file with connectivity information.")
...
def check_packmol_location(packmoldir, binary_name="packmol", dirname="packmoldir"):
    if packmoldir != None:
        ...
        print(BC.WARNING,
              f"No {dirname} argument passed. Attempting to find {dirname} variable in ASH settings file (~/ash_user_settings.ini)",
              BC.END)
        try:
            finaldir = ash.settings_ash.settings_dict[dirname]
            print(BC.OKGREEN, f"Using {dirname} path provided from ASH settings file (~/ash_user_settings.ini): ",
                  finaldir, BC.END)
            ...
            try:
                ...
                if finaldir:
                    print(BC.OKGREEN, f"Found {binary_name} binary in PATH. Using the following directory:", finaldir,
                          BC.END)
                else:
                    raise TypeError
            except TypeError:
