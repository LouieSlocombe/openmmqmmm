...
def Dynamics_ASE(fragment=None, PBC=False, theory=None, temperature=300, timestep=None, thermostat=None,
                 simulation_steps=None, simulation_time=None,
                 barostat=None, trajectoryname="Trajectory_ASE", traj_frequency=1, coupling_freq=0.002,
                 frozen_atoms=None, frozen_bonds=None,
                 frozen_angles=None, frozen_dihedrals=None, plumed_object=None, multiple_walkers=False, numwalkers=None,
                 ttime_nosehoover=5, safires=False, safires_solute=None, safires_inner_region=None,
                 safires_solvent_atomsnum=3,
    ...
    class ASHcalc(Calculator):
        ...
        def get_forces(self, atomsobj):
            ...
            # Calculate E+G
            result = Singlepoint(theory=self.theory, fragment=self.fragment, Grad=True, charge=self.charge,
                                 mult=self.mult)
    ...
    if thermostat == None and barostat == None:
        ...
        # trajectory=trajectoryname+'.traj'
        # , logfile='md.log'
        dyn = Langevin(atoms, timestep_fs * units.fs, friction=friction_coeff * units.fs, temperature_K=temperature,
                       logfile='md.log')
        ...
        # trajectory=trajectoryname+'.traj',
        dyn = NVTBerendsen(atoms, timestep_fs * units.fs, temperature, taut=coupling_freq * 1000 * units.fs,
                           logfile='md.log')
        ...
        dyn = NPT(atoms, externalstress=None, timestep=timestep_fs * units.fs, ttime=ttime_nosehoover * units.fs,
    ...
    if multiple_walkers == True:
        if numwalkers == 1:
            print("Please provide number of walkers by numwalkers keyword argument");
            ashexit()
        else:
            ...
            print("Launching")
            pool.apply_async(dynamics_walker, kwds=dict(atomsobj=atoms, simulation_steps=simulation_steps),
