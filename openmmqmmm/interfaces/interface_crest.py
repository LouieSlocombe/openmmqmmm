...
from ash.functions.functions_general import ashexit, BC, int_ranges, listdiff, print_line_with_subheader1, \
    print_time_rel, pygrep, check_program_location
...
def call_crest(fragment=None, xtbmethod=None, crestdir=None, charge=None, mult=None, solvent=None, energywindow=6,
    ...
    if crestdir == None:
        print(BC.WARNING,
              "No crestdir argument passed to call_crest. Attempting to find crestdir variable inside settings_ash",
              BC.END)
        try:
...
def call_crest_entropy(fragment=None, crestdir=None, charge=None, mult=None, numcores=1):
    ...
    process = sp.Popen(
        [crestdir + '/crest', 'initial.xyz', '--entropy', '-T', str(numcores), '-chrg', str(charge), '-uhf',
         str(mult - 1)],
