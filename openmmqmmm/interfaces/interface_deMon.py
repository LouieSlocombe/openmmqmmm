...
class deMon2kTheory:
    def __init__(self, demondir=None, filename='deMon', binary_name='binary', printlevel=2, numcores=1, label="deMon2k",
        ...
        if demondir == None:
            print(BC.WARNING,
                  f"No demondir argument passed to {self.theorynamelabel}Theory. Attempting to find demondir variable inside settings_ash",
                  BC.END)
            try:
...
def run_deMon2k(demondir, bin_name, filename, numcores=1):
    with open(filename + '.out', 'w') as ofile:
        if numcores > 1:
            process = sp.run(["mpirun", "--bind-to", "none", f"-np", f"{str(numcores)}", f"{demondir}/{bin_name}",
                              f"{filename}.inp"], check=True, stdout=ofile, stderr=ofile, universal_newlines=True)
        else:
            process = sp.run([demondir + f'/{bin_name}', filename + '.inp'], check=True, stdout=ofile, stderr=ofile,
