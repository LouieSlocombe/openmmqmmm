    ...
    with open(hessfile) as hfile:
        for line in hfile:
            if cartgrab == True:
                count = count + 1
                elem = line.split()[0];
                x_c = openmmqmmm.constants.bohr2ang * float(line.split()[2]);
                y_c = openmmqmmm.constants.bohr2ang * float(line.split()[3]);
                z_c = openmmqmmm.constants.bohr2ang * float(line.split()[4])
...
H    2.453295744  -1.445998564  -1.389381355
    ...
    print("\nUncorrected binding energy: {} kcal/mol".format(
        (dimer_energy - monomer1_energy - monomer2_energy) * openmmqmmm.constants.hartokcal))
    ...
    #
    deltaE_unc = (dimer_energy - monomer1_energy - monomer2_energy) * openmmqmmm.constants.hartokcal
    counterpoise_corr = -1 * (
            monomer1_in_dimer_dimerbasis_energy - monomer1_in_dimergeo_energy + monomer2_in_dimer_dimerbasis_energy - monomer2_in_dimergeo_energy) * openmmqmmm.constants.hartokcal
...
def orca_frag_guess(fragment=None, theory=None, A_indices=None, B_indices=None, A_charge=None, B_charge=None,
    ...
    fragment_A = openmmqmmm.Fragment(elems=elems_A, coords=coords_A, charge=A_charge, mult=A_mult)
    fragment_B = openmmqmmm.Fragment(elems=elems_B, coords=coords_B, charge=B_charge, mult=B_mult)
    ...
    print("-" * 120)
    # Run A SP
    result_calcA = openmmqmmm.Singlepoint(theory=calc_A, fragment=fragment_A)
    ...
    print("-" * 120)
    # Run B SP
    result_calcB = openmmqmmm.Singlepoint(theory=calc_B, fragment=fragment_B)
...
{extrablock}
"""
        rohf = openmmqmmm.ORCATheory(orcasimpleinput=f"! ROHF {basis} tightscf notrah  {extrainput}", orcablocks=rohfblocks,
...
"""
        avas_prep = openmmqmmm.ORCATheory(orcasimpleinput=f"!  {basis} {extrainput} noiter {AVAS_keyword}",
...
"""
        mp2_prep = openmmqmmm.ORCATheory(orcasimpleinput=f"! RI-MP2 {basis} {extrainput} autoaux tightscf",
    ...
    if orbitals_option == "HF" or orbitals_option == "RHF":
        ...
        exit()
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} HF {basis}  tightscf", numcores=numcores,
        ...
        exit()
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} UHF {basis} normalprint UNO tightscf",
                                 numcores=numcores,
        ...
        exit()
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} UHF {basis}  UNO tightscf", numcores=numcores,
        ...
        if NMF_sigma is None:
            ...
            ashexit()
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} {basis}  tightscf", NMF=True, NMF_sigma=NMF_sigma,
                                 numcores=numcores,
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} MP2 {basis} autoaux tightscf", orcablocks=mp2blocks,
                                 numcores=numcores,
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} RI-MP2 {basis} autoaux tightscf",
                                 orcablocks=mp2blocks, numcores=numcores,
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} RI-SCS-MP2 {basis} autoaux tightscf",
                                 orcablocks=mp2blocks, numcores=numcores,
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} OO-RI-MP2 {basis} autoaux tightscf",
                                 orcablocks=mp2blocks, numcores=numcores,
        ...
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} {MDCIkeyword} {basis} autoaux tightscf",
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} CASSCF {basis} tightscf", orcablocks=casscfblocks,
                                 numcores=numcores,
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} {MRCIkeyword} {basis} autoaux tightscf",
        ...
        """
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput} {AUTOCIkeyword} {basis} autoaux tightscf",
...
"""
        ...
        natorbs = openmmqmmm.ORCATheory(orcasimpleinput=f"! {extrainput}  {basis} autoaux tightscf", orcablocks=mrciblocks,
                                 numcores=numcores,
    ...
    if alldone is False:
        ...
        if natorb_iterations:
            ...
            for n_i in range(0, natorb_iterations):
                ...
                openmmqmmm.Singlepoint(theory=natorbs, fragment=fragment, charge=charge, mult=mult)
...
def create_ORCA_FCIDUMP(gbwfile, header_format="FCIDUMP", filename="FCIDUMP_ORCA", orca_json_format="msgpack",
    ...
    from openmmqmmm.modules.module_coords import nuc_nuc_repulsion
    ...
    WF_assignment = openmmqmmm.functions.functions_elstructure.check_occupations(occupations)
    ...
    # Write file
    from openmmqmmm.functions.functions_elstructure import ASH_write_integralfile
...
def calculate_ORCA_natorbs_from_density(gbwfile, densityname="mdcip"):
    from openmmqmmm.functions.functions_elstructure import diagonalize_DM_AO
