...
def check_DRACO_location(dracodir):
    if dracodir != None:
        ...
        print(BC.WARNING,
              "No dracodir argument passed. Attempting to find dracodir variable in ASH settings file (~/ash_user_settings.ini)",
              BC.END)
        try:
            finaldracodir = ash.settings_ash.settings_dict["dracodir"]
            print(BC.OKGREEN, "Using dracodir path provided from ASH settings file (~/ash_user_settings.ini): ",
                  finaldracodir, BC.END)
        except KeyError:
