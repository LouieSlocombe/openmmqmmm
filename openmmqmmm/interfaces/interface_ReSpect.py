...
class ReSpectTheory:
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        create_respect_inputfile(self.filename, qm_elems, current_coords, charge, mult, jobtype=self.jobtype,
                                 scf_inputkeywords=self.scf_inputkeywords,
                                 jobtype_inputkeywords=self.jobtype_inputkeywords, )

        # Always run SCF-job first
        print("Running ReSpect SCF calculation")
        run_respect(respectdir=self.respectdir, jobtype='scf', inputfile=self.filename, numcores=self.numcores,
                    scratchdir=f'{os.getcwd()}/respect_calc_scratch')
        ...
        if self.jobtype is not None:
            ...
            if isinstance(self.jobtype, list):
                ...
                for job in self.jobtype:
                    ...
                    run_respect(respectdir=self.respectdir, jobtype=self.jobtype, inputfile=self.filename,
                                numcores=self.numcores, scratchdir=f'{os.getcwd()}/respect_calc_scratch')
                    # Grab properties
                    if job == "gt":
...
def run_respect(respectdir=None, jobtype='scf', inputfile='', numcores=1, scratchdir='.'):
    with open('respect_.out', 'w') as ofile:
        process = sp.run([respectdir + f'/respect', f'--{jobtype}', f'--inp={inputfile}', f"--nt={numcores}",
...
def create_respect_inputfile(filename, elems, coords, charge, mult, scf_inputkeywords=None, jobtype_inputkeywords=None,
                             jobtype=None):
    indent = "  "
