...
class BlockTheory:
    def __init__(self, blockdir=None, pyscftheoryobject=None, blockversion='Block2', filename='input.dat', printlevel=2,
        ...
        if self.groupname is not None:
            ...
            if orbsym is None:
                print(
                    "Error: orbsym keyword should be set. should be list of integers representing irrep ids of each orb")
                ashexit()
            self.orbsym = orbsym
        ...
        if self.block_parallelization == 'MPI':
            ...
            if self.numcores != self.hybrid_num_mpi_procs * self.hybrid_num_threads:
                print(
                    f"Error: numcores={self.numcores} (This is total number of cores available) is not equal to hybrid_num_mpi_procs({self.hybrid_num_mpi_procs})*hybrid_num_threads({self.hybrid_num_threads}) = {self.hybrid_num_mpi_procs * self.hybrid_num_threads}")
                ashexit()
...
"""
    ...
    def load_dmrgscf(self):
        ...
        try:
            ...
            print(
                "Once dmrgscf is installed you have to create settings.py file (see path in pyscf import error message, probably above) and add this:")
            m = f"""
...
MPIPREFIX = "" # mpi-prefix. Best to leave blank
    ...
    def call_block_directly(self):
        ...
        if self.block_parallelization == 'MPI':
            ...
            with open('output.dat', "w") as outfile:
                sp.call(['mpirun', '-np', str(self.numcores), ' --bind-to none', self.block_binary, self.filename],
                        stdout=outfile)
        else:
    ...
    def setup_initial_orbitals(self, elems):
        ...
        if self.moreadfile is None:
            ...
            if self.initial_orbitals not in ['RKS', 'UKS', 'RHF', 'UHF', 'canMP2', 'MP2', 'DFMP2', 'DFMP2relax', 'CCSD',
                                             'CCSD(T)', 'DMRG', 'AVAS-CASSCF', 'DMET-CASSCF', 'CASSCF']:
    ...
    def setup_active_space(self, occupations=None):
        ...
        if self.active_space != None:
            ...
            print(
                f"Active space determined from occupation threshold parameters: cas_nmin={self.cas_nmin} and cas_nmax={self.cas_nmax}")
    ...
    def setup_DMRG_job(self, verbose=5, rdmoption=None):
        ...
        if self.macroiter == 0:
            ...
            #
            self.mch = self.dmrgscf.DMRGSCF(self.pyscftheoryobject.mf, self.norb, self.nelec, maxM=self.maxM,
                                            tol=self.tol)
            ...
            if self.runcalls == 1:
                ...
                # Regular HF orbitals in mf object
                    ...
                    if self.active_space is None:
                        print(
                            "Error: initial_orbitals=HF option requires active_space to be set : e.g. active_space=[2,10] for 2 el in 10 orbs")
                        ashexit()
                    self.nelec = self.active_space[0]
                    self.norb = self.active_space[1]
        ...
        if self.Block_direct:
            ...
            try:
                ...
                self.pyscftheoryobject.run_population_analysis(self.pyscftheoryobject.mf, unrestricted=False,
                                                               dm=rdm1_MO, type='Mulliken', label='DMRG')
                pyscf.scf.uhf.mulliken_spin_pop(self.pyscftheoryobject.mol, rdm1s,
