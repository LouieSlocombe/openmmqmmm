...
class TurbomoleTheory:
    def __init__(self, TURBODIR=None, turbomoledir=None, filename='XXX', printlevel=2, label="Turbomole",
        ...
        if numcores != 1:
            print(
                f"Parallel job requested with numcores: {numcores} . Make sure that the correct OpenMPI version is available in your environment")
        ...
        if TURBODIR is not None:
            ...
            print(BC.WARNING,
                  f"No turbomoledir argument passed to {self.theorynamelabel}Theory. Attempting to find turbomoledir variable inside settings_ash",
                  BC.END)
            try:
                ...
                try:
                    ...
                    print(BC.OKGREEN,
                          "Found ridft (Turbomol executable) in PATH. Setting turbomoledir to parentdir of that dir:",
                          self.turbomoledir, BC.END)
                except:
    ...
    def cleanup(self):
        files = ['coord', 'control', 'energy', 'gradient', 'auxbasis', 'basis', 'mos', 'ridft.out', 'rdgrad.out',
                 'ricc2.out', 'statistics']
        for f in files:
            if os.path.exists(f):
                os
        ...
        with open(filename + '.out', 'w') as ofile:
            if numcores > 1:
                if parallelization == 'MPI':
                    ...
                    process = sp.run([f"{self.TURBODIR}/bin/{self.sysname}" + f'/{exe}'], check=True, stdout=ofile,
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        # Create controlfile
            ...
            create_control_file(functional=self.functional, gridsize=self.gridsize, scfconv=self.scfconv, dft=self.dft,
                                ...
                                scfiterlimit=self.scfiterlimit, maxcor=self.maxcor, ricore=self.ricore, charge=charge,
                                mult=mult,
                                pcharges=MMcharges, pccoords=current_MM_coords, pointcharge_type=self.pointcharge_type,
                                pc_gaussians=self.pc_gaussians)
        ...
        if self.mp2 is True:
            ...
            self.run_turbo(self.turbomoledir, self.filename_mp2, exe=self.turbo_mp2_exe,
                           parallelization=self.parallelization, numcores=self.numcores)
    ...
    with open(filename, 'w') as coordfile:
        ...
        for i in range(len(elems)):
            if write_unit.upper() == "FRACT":
                coordfile.write(
                    f"{coords[i][0] / periodic_info[0]} {coords[i][1] / periodic_info[1]} {coords[i][2] / periodic_info[2]} {elems[i]}\n")
            else:
                coordfile.write(
                    f"{coords[i][0] * conversion_factor} {coords[i][1] * conversion_factor} {coords[i][2] * conversion_factor} {elems[i]}\n")
        # PBC
        if periodic_info is not None:
            coordfile.write(f"$periodic 3\n")
            coordfile.write(f"$cell {unit_string}\n")
            coordfile.write(
                f"{periodic_info[0]} {periodic_info[1]} {periodic_info[2]} {periodic_info[3]} {periodic_info[4]} {periodic_info[5]}\n")
        coordfile.write("$end\n")


def create_control_file(functional="lh12ct-ssifpw92", gridsize="m4", scfconv="7", symmetry="c1", rij=True, dft=True,
                        mp2=False,
                        basis="def2-SVP", jbasis="def2-SVP", scfiterlimit=30, maxcor=500, ricore=500, charge=None,
                        mult=None,
