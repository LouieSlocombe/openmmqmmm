...
from ash.constants import harkcal
...
class DiceTheory:
    def __init__(self, dicedir=None, pyscftheoryobject=None, filename='input.dat', printlevel=2, numcores=1,
                 ...
                 QMC_trialWF=None, QMC_SHCI_numdets=1000, QMC_dt=0.005, QMC_nsteps=50, QMC_nblocks=1000,
                 QMC_nwalkers_per_proc=5):
        ...
        if dicedir == None:
            print(BC.WARNING,
                  f"No dicedir argument passed to {self.theorynamelabel}Theory. Attempting to find dicedir variable inside settings_ash",
                  BC.END)
            try:
        ...
        if numcores > 1:
            try:
                print(
                    f"MPI-parallel job requested with numcores: {numcores} . Make sure that the correct OpenMPI version is available in your environment")
                check_OpenMPI()
            except:
...
noio
    ...
    def setup_initial_orbitals(self, elems):
        ...
        if self.moreadfile == None:
            ...
            if self.initial_orbitals not in ['RKS', 'UKS', 'RHF', 'UHF', 'canMP2', 'MP2', 'DFMP2', 'DFMP2relax', 'CCSD',
                                             'CCSD(T)', 'SHCI', 'AVAS-CASSCF', 'DMET-CASSCF', 'CASSCF']:
            ...
            if self.initial_orbitals == 'SHCI':
                ...
                MP2nat_occupations, MP2nat_mo_coefficients = self.pyscftheoryobject.calculate_natural_orbitals(
                    self.pyscftheoryobject.mol,
                ...
                self.setup_SHCI_job(verbose=5,
                                    rdmoption=True)  # Creates the self.mch CAS-CI/CASSCF object with RDM True
                ...
                occupations, mo_coefficients = self.pyscftheoryobject.calculate_natural_orbitals(
                    self.pyscftheoryobject.mol,
        ...
        if True in [i > 2.00001 for i in occupations]:
            print(
                "Warning! Occupation array contains occupations larger than 2.0. Something possibly wrong (bad convergence?)")
            print("Continuing but these orbitals may be bad")
            # ashexit()
        if True in [i < 0.0 for i in occupations]:
            print(
                "Warning!. Occupation array contains negative occupations. Something possibly wrong (bad convergence?)")
            print("Continuing but these orbitals may be bad")
        #    ashexit()
    ...
    def setup_active_space(self, occupations=None):
        ...
        if self.SHCI_active_space != None:
            ...
            print(
                f"SHCI Active space determined from {self.initial_orbitals} NO threshold parameters: SHCI_cas_nmin={self.SHCI_cas_nmin} and SHCI_cas_nmax={self.SHCI_cas_nmax}")
            ...
            indices_for_thresholds = [i for i, j in enumerate(occupations) if
                                      j < self.SHCI_cas_nmin and j > self.SHCI_cas_nmax]
            ...
            print(
                f"To get this same active space in another calculation you can also do: \nSHCI_active_space=[{self.nelec},{self.norb}]")
    ...
    def SHCI_object_run(self, write_det_CASCI=False, numdets=None):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"This either means you requested no stochastic PT or that stochastic PT steps exceeded PT iterations {self.SHCI_PTiter}")
    ...
    def run(self, current_coords=None, current_MM_coords=None, MMcharges=None, qm_elems=None, mm_elems=None,
        ...
        if charge == None or mult == None:
            print(BC.FAIL, f"Error. charge and mult has not been defined for {self.theorynamelabel}Theory.run method",
                  BC.END)
            ashexit()
        ...
        if self.NEVPT2 is True:
            ...
            mo_coeffs, occupations = self.setup_initial_orbitals(
                elems)  # Returns mo-coeffs and occupations of initial orbitals
        ...
        # AFQMC
            ...
            if self.QMC_trialWF == 'SHCI':
                ...
                mo_coeffs, occupations = self.setup_initial_orbitals(
                    elems)  # Returns mo-coeffs and occupations of initial orbitals
                self.setup_active_space(
                    occupations=occupations)  # This will define self.norb and self.nelec active space
                self.setup_SHCI_job()  # Creates the self.mch CAS-CI/CASSCF object
                self.SHCI_object_set_mos(mo_coeffs=mo_coeffs)  # Sets the MO coeffs of mch object
                self.SHCI_object_run(write_det_CASCI=True,
                                     numdets=self.QMC_SHCI_numdets)  # Runs the self.mch object with dets-printout
                ...
                if self.QMC_SHCI_numdets > self.num_var_determinants:
                    print(
                        f"Error: QMC_SHCI_numdets ({self.QMC_SHCI_numdets}) larger than SHCI-calculated determinants ({self.num_var_determinants})")
                    print(
                        "Increase SHCI-WF size: e.g. by increasing active space(SHCI_cas_nmin, SHCI_cas_nmax or SHCI_active_space) or reducing thresholds (SHCI_sweep_epsilon)")
                    ashexit()
                ...
                e_afqmc, err_afqmc = self.QMCUtils.run_afqmc_mc(self.mch, mpi_prefix=f"mpirun -np {numcores}",
                                                                ...
                                                                dt=self.QMC_dt, steps_per_block=self.QMC_nsteps,
                                                                nwalk_per_proc=self.QMC_nwalkers_per_proc,
            ...
            # Single determinant trial wavefunction
                ...
                module_init_time = time.time()
                e_afqmc, err_afqmc = self.QMCUtils.run_afqmc_mf(self.pyscftheoryobject.mf,
                                                                mpi_prefix=f"mpirun -np {numcores}",
                                                                norb_frozen=self.frozen_core_orbs, chol_cut=1e-5,
                                                                seed=None, dt=self.QMC_dt,
                                                                steps_per_block=self.QMC_nsteps,
                                                                nwalk_per_proc=self.QMC_nwalkers_per_proc,
                                                                nblocks=self.QMC_nblocks,
            ...
            if self.SHCI is True:
                ...
                if self.SHCI_DoRDM is True:
                    ...
                    self.pyscftheoryobject.write_orbitals_to_Moldenfile(self.pyscftheoryobject.mol,
                                                                        mo_coefficients, occupations,
                                                                        label="SHCI_Final_nat_orbs")
                    # RDM
                    ...
                    try:
                        ...
                        self.pyscftheoryobject.run_population_analysis(self.pyscftheoryobject.mf, unrestricted=False,
                                                                       dm=rdm1, type='Mulliken', label='SHCI')
                        import pyscf
                        pyscf.scf.uhf.mulliken_spin_pop(self.pyscftheoryobject.mol, dm_ab,
                                                        s=self.pyscftheoryobject.mf.get_ovlp(), verbose=3)
                    except:
                        pass
                    # Dipole moment
                    print("Now doing dipole")
                    dipole = self.pyscftheoryobject.get_dipole_moment(dm=rdm1,
                                                                      label=f"{self.label}_SHCI_eps_{self.SHCI_sweep_epsilon[-1]}")
