...
from ash.functions.functions_general import BC, ashexit, print_line_with_mainheader
...
class DualTheory:
    ...
    def __init__(self, theory1=None, theory2=None, printlevel=1, label=None, correctiontype="Difference",
                 update_schedule='frequency', update_freq=5, max_updates=100, numcores=1, Maxthreshold=3e-5,
                 minimum_steps=7):
    ...
    def run(self, current_coords=None, elems=None, Grad=False, numcores=None, label='default', charge=None, mult=None,
            run_both_theories=False,
        ...
        if self.theory2_active == True:
            ...
            if Grad == True:
                T2_energy, T2_grad = self.theory2.run(current_coords=current_coords, elems=elems, numcores=numcores,
                                                      Grad=True, charge=charge, mult=mult)
                return T2_energy, T2_grad
            else:
                T2_energy = self.theory2.run(current_coords=current_coords, elems=elems, numcores=numcores, Grad=False,
        ...
        # If theory1 switch is active then we only do theory1
            ...
            if Grad == True:
                T1_energy, T1_grad = self.theory1.run(current_coords=current_coords, elems=elems, numcores=numcores,
                                                      Grad=True, charge=charge, mult=mult)
                return T1_energy, T1_grad
            else:
                T1_energy = self.theory1.run(current_coords=current_coords, elems=elems, numcores=numcores, Grad=False,
                                             charge=charge, mult=mult)
                return T1_energy
        # Else we do theory1 with regular theory2 correction
            ...
            if self.update_schedule == 'Robust_start':
                ...
                if self.correction_off == True:
                    ...
                    print("self.correction_dict[label].Ecorr:", self.correction_dict[label].correction_E)
                    print("self.correction_dict[label].correction_RMSgrad:",
                          self.correction_dict[label].correction_RMSgrad)
                    print("self.correction_dict[label].correction_Maxgrad:",
                          self.correction_dict[label].correction_Maxgrad)
                    try:
                        delta_Max = self.correction_dict[label].correction_Maxgrad[-2] - \
                                    self.correction_dict[label].correction_Maxgrad[-1]
                        delta_RMS = self.correction_dict[label].correction_RMSgrad[-2] - \
                                    self.correction_dict[label].correction_RMSgrad[-1]
            ...
            if Grad == True:
                ...
                if run_both_theories == True:
                    ...
                    self.update_freq_dict[label][2] += 1
                    self.write_updatefreqdict_to_disk(self.update_freq_dict)
                    T2_energy = self.theory2.run(current_coords=current_coords, elems=elems, numcores=numcores,
                                                 Grad=False, charge=charge, mult=mult)
                    # Update gradient correction if both theories were calculated
                    self.correction(Grad, label, T2_E=T2_energy, T1_E=T1_energy, T2_G=T2_grad, T1_G=T1_grad)
                    # self.correction_stats(label)
    ...
    def __init__(self, theory1=None, theory2=None, theories=None, printlevel=2, label=None,
        ...
        if theories is not None:
            ...
            if theory1 is not None or theory2 is not None:
                ...
                print(
                    "Choose either to define: theory1 and theory2       or provide a list of all theories using the theories keyword")
                exit()
        else:
        ...
        if self.theory_operators is not None:
            ...
            if len(self.theory_operators) != len(self.theories):
                print(
                    f"Error: Number of theory-operators {len(self.theory_operators)} is not equal to number of theories {len(self.theories)}")
                ashexit()
