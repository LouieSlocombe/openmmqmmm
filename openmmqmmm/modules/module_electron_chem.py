...
def ElectronImpact(fragment=None, theory=None, HLtheory=None, charge=None, mult=None, qcxmsdir=None, xtbmethod=None,
                   numcores=1):
    ...
    print("-" * 50)
    print("Now starting optimization")
    geomeTRICOptimizer(fragment=fragment, theory=theory, maxiter=500, coordsystem='hdlc', charge=cation_charge,
                       mult=cation_mult)
    # print("Now starting Numerical frequencies")
    ...
    print("\nNow calling QCxMS")
    # TODO: THIS IS NOT YET READY
    unique_fragments_reactions = call_QCxMS(fragment=fragment, qcxmsdir=qcxmsdir, xtbmethod=xtbmethod, charge=charge,
                                            mult=mult, numcores=numcores, ntraj=300, tmax=10, tinit=500, ieeatm=0.6)
    ...
    for reaction in unique_fragments_reactions:
        ...
        for molfrag in reaction:
            ...
            geomeTRICOptimizer(theory=theory, fragment=molfrag, maxiter=500, coordsystem='hdlc', charge=molfrag.charge,
                               mult=molfrag.mult)
            thermochem = NumFreq(fragment=molfrag, theory=theory, npoint=2, runmode='serial', charge=molfrag.charge,
                                 mult=molfrag.mult)
...
def call_QCxMS(fragment=None, qcxmsdir=None, xtbmethod=None, charge=None, mult=None, numcores=1, ntraj=300, tmax=10,
               tinit=500, ieeatm=0.6):
    print_line_with_subheader1("call_QCxMS")
    if qcxmsdir == None:
        print(BC.WARNING,
              "No qcxmsdir argument passed to call_crest. Attempting to find qcxmsdir variable inside settings_ash",
              BC.END)
        try:
