...
from openmmqmmm.functions.functions_general import ashexit, blankline, print_time_rel, BC, load_julia_interface
from openmmqmmm.modules.module_coords import distance
from openmmqmmm.modules.module_theory import Theory
...
class NonBondedTheory(Theory):
    def __init__(self, atomtypes=None, forcefield=None, charges=None, LJcombrule='geometric',
        ...
        if codeversion == None:
            codeversion = openmmqmmm.settings_ash.settings_dict["nonbondedMM_code"]
    ...
    def run(self, current_coords=None, elems=None, charges=None, connectivity=None, numcores=1, label=None,
        ...
        if self.codeversion == 'py':
            ...
            if Coulomb:
                ...
                if self.printlevel >= 2:
                    ...
                    print("Coulomb Energy (kcal/mol):", self.Coulombchargeenergy * openmmqmmm.constants.harkcal)
                    print("")
        ...
        if self.printlevel >= 2:
            ...
            print("Lennard-Jones Energy (kcal/mol):", self.LJenergy * openmmqmmm.constants.harkcal)
            print("Coulomb Energy (au):", self.Coulombchargeenergy)
            print("Coulomb Energy (kcal/mol):", self.Coulombchargeenergy * openmmqmmm.constants.harkcal)
...
def LennardJones(coords, epsij, sigmaij, connectivity=None, qmatoms=None):
    ...
    final_gradient = gradient * (1 / openmmqmmm.constants.harkcal) / openmmqmmm.constants.ang2bohr
    # print("LJ gradient (hartree/Bohr):", final_gradient)
    # Converg energy from kcal/mol to hartree
    final_energy = energy * (1 / openmmqmmm.constants.harkcal)
                    ...
                    for l in LJPairpotentials:
                        ...
                        if set([atomtypes[i], atomtypes[j]]) == set([l[0], l[1]]):
                            ...
                            if skip == False:
                                ...
                                eps = l[3]
                                # pairdistance = distance(coords[i], coords[j])
                                pairdistance = pairdistance_b * openmmqmmm.constants.bohr2ang
                                # print("sigma, eps, pairdistance", sigma,eps,pairdistance)
                                V_LJ = 4 * eps * ((sigma / pairdistance) ** 12 - (sigma / pairdistance) ** 6)
                                # print("V_LJ: {} kcal/mol  V_LJ: {} au:".format(V_LJ,V_LJ/openmmqmmm.constants.harkcal))
                                LJenergy += V_LJ
                                # print("energy: {} kcal/mol  energy: {} au:".format(energy, energy / openmmqmmm.constants.harkcal))
                                # print("------------------------------")
                                # Typo in http://localscf.com/localscf.com/LJPotential.aspx.html ??
                                # Using http://www.courses.physics.helsinki.fi/fys/moldyn/lectures/L4.pdf
                                ...
                                LJgradient[j] -= gr
    # Convert gradient from kcal/mol per Å to hartree/Bohr
    LJfinal_gradient = LJgradient * (1 / openmmqmmm.constants.harkcal) / openmmqmmm.constants.ang2bohr
    # print("LJ gradient (hartree/Bohr):", LJfinal_gradient)
    # Converg energy from kcal/mol to hartree
    LJfinal_energy = LJenergy * (1 / openmmqmmm.constants.harkcal)
...
def seminario(fragment=None, hessian=None, method="original"):
    ...
    coords_au = fragment.coords * openmmqmmm.constants.ang2bohr
    ...
    def do_bonds(list_of_bonds, coords_au, hessian):
        ...
        for bond in list_of_bonds:
            ...
            factor = openmmqmmm.constants.harkcal / (openmmqmmm.constants.bohr2ang ** 2)
    ...
    def do_angles(list_of_angles, coords_au, hessian, dist_matrix):
        ...
        for angle in list_of_angles:
            ...
            factor = openmmqmmm.constants.harkcal
            print("factor:", factor)
            fc_kcal = abs(fc_au * factor)
    ...
    print("")
    # Do angles
    list_of_angles = [[1, 0, 2],
                      [0, 1, 2]]  # Placeholder. To be replaced by actual angle list from fragment connectivity
