...
class QMMMTheory:
    def __init__(
        ...
        print(f"MM region ({len(self.mmatoms)} atoms)")
        ...
        # Default 1 proc
            ...
            self.numcores = 1
        print(f"QM/MM object selected to use {self.numcores} cores")
        ...
        if mm_theory is not None:
            ...
            if self.printlevel > 2:
                for i in self.allatoms:
                    if i in self.qmatoms:
                        if self.embedding.lower() == "elstat":
                            print(f"QM atom {i} ({self.elems[i]}) charge: {self.charges_qmregionzeroed[i]}")
                        else:
                            print(f"QM atom {i} ({self.elems[i]}) charge: {self.charges[i]}")
                    else:
                        if self.printlevel > 3:
                            print(f"MM atom {i} ({self.elems[i]}) charge: {self.charges_qmregionzeroed[i]}")
            blankline()
        else:
            ...
            self.dipole_correction = False
        print_time_rel(
            module_init_time, modulename="QM/MM object creation", currprintlevel=self.printlevel, moduleindex=3
        )
    ...
    def SetDipoleCharges(self, current_coords):
        ...
        print_time_rel(
            checkpoint, modulename="SetDipoleCharges", moduleindex=3, currprintlevel=self.printlevel, currthreshold=2
        )
    ...
    def TruncatedPCfunction(self, used_qmcoords):
        ...
        if self.TruncatedPCcalls == 1 or self.TruncatedPCcalls % self.TruncatedPC_recalc_iter == 0:
            ...
            print(f"Truncated PC-region size: {len(self.truncated_PC_region_indices)} charges")
            ...
            self.TruncatedPC_recalc_flag = False
            print(
                f"This is QM/MM run no. {self.TruncatedPCcalls}. Using approximate truncated PC field: {len(self.truncated_PC_region_indices)} charges"
            )
    ...
    def calculate_truncPC_gradient_correction(
    ...
    def mech_run(
        ...
        if self.printlevel >= 2:
            print(f"Running QM/MM object with {numcores} cores available")

        ################
        # QMTheory.run
        ################
        print_time_rel(
            module_init_time, modulename="before-QMstep", moduleindex=2, currprintlevel=self.printlevel, currthreshold=1
        )
        CheckpointTime = time.time()
        ...
        print_time_rel(
            CheckpointTime, modulename="QM step", moduleindex=2, currprintlevel=self.printlevel, currthreshold=1
        )
        CheckpointTime = time.time()
        ...
        if Grad:
            ...
            print_time_rel(
                CheckpointTime,
                modulename="linkatomgrad prepare",
                moduleindex=2,
                currprintlevel=self.printlevel,
                currthreshold=1,
            )
            print_time_rel(
                Grad_prep_CheckpointTime,
                modulename="QM/MM gradient prepare",
                moduleindex=2,
                currprintlevel=self.printlevel,
                currthreshold=1,
            )
        ...
        if self.mm_theory_name == "OpenMMTheory":
            ...
            if Grad:
                ...
                if self.openmm_externalforce is True:
                    print_if_level("OpenMM externalforce is True", self.printlevel, 2)
                    ...
                    if exit_after_customexternalforce_update is True:
                        print_if_level("OpenMM custom external force updated. Exit requested", self.printlevel, 2)
            ...
            self.MMenergy = 0
        print_time_rel(
            CheckpointTime, modulename="MM step", moduleindex=2, currprintlevel=self.printlevel, currthreshold=1
        )
        CheckpointTime = time.time()
    ...
    def create_linkatoms(self, current_coords):
        ...
        print_time_rel(
            checkpoint, modulename="create_linkatoms", moduleindex=3, currprintlevel=self.printlevel, currthreshold=2
        )
    ...
    def runprep(self, current_coords):
            ...
            if self.embedding.lower() == "elstat":
                ...
                if self.chargeboundary_method == "shift":
                    ...
                    if self.dipole_correction is True:
                        ...
                        if self.printlevel > 1:
                            print(f"Adding {len(self.dipole_charges)} dipole charges to PC environment")
                        ...
                        print_time_rel(
                            check_before_linkatoms,
                            modulename="Linkatom-dipolecorrection",
                            moduleindex=3,
                            currprintlevel=self.printlevel,
                            currthreshold=2,
                        )
                        ...
                        if self.printlevel > 1:
                            print("Number of pointcharges: ", len(self.pointcharges))
                # RCD
        ...
        print_time_rel(
            init_time_runprep, modulename="runprep", moduleindex=3, currprintlevel=self.printlevel, currthreshold=2
        )
    ...
    def elstat_run(
        ...
        if Grad is True:
            ...
            if self.printlevel >= 2:
                print(BC.WARNING, BC.BOLD, "------------ENDING QM/MM MODULE-------------", BC.END)
            print_time_rel(
                module_init_time, modulename="QM/MM run", moduleindex=2, currprintlevel=self.printlevel, currthreshold=1
            )
            return self.QM_MM_energy, self.QM_MM_gradient
        else:
            print_time_rel(
...
def actregiondefine(pdbfile=None, mmtheory=None, psffile=None, fragment=None, radius=None, originatom=None):
    ...
    print("Radius:", radius)
    print(f"Origin atom: {originatom} ({fragment.elems[originatom]})")
    print(f"Will find all atoms within {radius} Å from atom: {originatom} ({fragment.elems[originatom]})")
...
def compute_decomposed_QM_MM_energy(fragment=None, theory=None):
    ...
    theory.printlevel = 0
    result = openmmqmmm.Singlepoint(theory=theory, fragment=fragment, printlevel=0)
    ...
    result_MM_mod2 = openmmqmmm.Singlepoint(theory=theory.mm_theory, fragment=fragment, charge=0, mult=1, printlevel=0)
    ...
    result_mech = openmmqmmm.Singlepoint(theory=QM_MM_mech, fragment=fragment, printlevel=0)
