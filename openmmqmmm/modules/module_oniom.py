...
from ash.interfaces.interface_xtb import grabatomcharges_xTB, xTBTheory
from ash.modules.module_QMMM import linkatom_force_adv, linkatom_force_lever, linkatom_force_chainrule
from ash.modules.module_coords import print_coords_for_atoms, get_boundary_atoms, get_linkatom_positions, distance, \
    get_connected_atoms
...
class ONIOMTheory(Theory):
    def __init__(self, theories_N=None, regions_N=None, regions_chargemult=None,
        ...
        if fullregion_charge is None or fullregion_mult is None:
            print(
                "Error: Full-region charge and multiplicity must be provided (fullregion_charge, fullregion_mult keywords)")
            ashexit()
        ...
        if regions_chargemult is None:
            print(
                "Error: regions_chargemult must be provided for N-layer ONIOM (list of lists of charge,mult for each region)")
            ashexit()
        ...
        if len(self.theories_N) == 2:
            ...
            print()
            self.boundaryatoms = get_boundary_atoms(self.regions_N[0], self.fragment.coords, self.fragment.elems,
                                                    conn_scale,
                                                    conn_tolerance, excludeboundaryatomlist=excludeboundaryatomlist,
                                                    unusualboundary=None)
        elif len(self.theories_N) == 3:
            self.theorylabels = ["HL", "IL", "LL"]
            # Atom labels
            atomlabels = ["HL" if b in self.regions_N[0] else "ML" if b in self.regions_N[1] else "LL" for b in
                          self.fragment.allatoms]
            print("atomlabels:", atomlabels)
            # If HL-LL covalent boundary issue and ASH exits then printing QM-coordinates is useful
            print("\nFull-system coordinates (before any linkatoms):")
            print_coords_for_atoms(self.fragment.coords, self.fragment.elems, self.fragment.allatoms, labels=atomlabels)
            #
            self.boundaryatoms_HL_ML = get_boundary_atoms(self.regions_N[0], self.fragment.coords, self.fragment.elems,
                                                          conn_scale,
                                                          conn_tolerance,
                                                          excludeboundaryatomlist=excludeboundaryatomlist,
                                                          unusualboundary=None)
            print("boundaryatoms_HL_ML:", self.boundaryatoms_HL_ML)
            self.boundaryatoms_ML_LL = get_boundary_atoms(self.regions_N[1], self.fragment.coords, self.fragment.elems,
                                                          conn_scale,
                                                          conn_tolerance,
                                                          excludeboundaryatomlist=excludeboundaryatomlist,
                                                          unusualboundary=None)
    ...
    def run(self, current_coords=None, Grad=False, elems=None, charge=None, mult=None, label=None, numcores=None):
        ...
        for j, region in enumerate(self.regions_N):
            ...
            for i, theory in enumerate(self.theories_N):
                ...
                if self.embedding.lower() == "elstat":
                    ...
                    if self.linkatoms is True:
                        ...
                        if self.chargeboundary_method.lower() == "chargeshift":
                            ...
                            pointcharges, pointchargecoords = self.RCD_shifting(charges_qmregionzeroed,
                                                                                pointchargecoords, current_coords,
                                                                                PCregion)
                    else:
                    ...
                    assert len(pointcharges) == len(
                        pointchargecoords), f"Num pointcharges ({len(pointcharges)}) and pointchargecoords ({len(pointchargecoords)}) do not match"
                ...
                print(
                    f"Running Theory {i + 1} ({theory.theorynamelabel}) on Region {j + 1} ({len(region_elems_final)} atoms)")
                ...
                if theory.theorytype == "MM":
                    ...
                    res = theory.run(current_coords=full_coords, elems=full_elems, Grad=Grad, numcores=theory.numcores,
                                     label=label)
                # if the theory is QM/MM then this
                elif theory.theorytype == "QM/MM":
                    print("Case: Theory is QM/MM object")
                    print(
                        "Warning: A subtractive correction for a region where the theory is a QM/MM Theory does not make much sense")
                    print("Warning: Using only the QM-theory part of the QM/MM object to calculate the subtraction")
                    res = theory.qm_theory.run(current_coords=region_coords_final, elems=region_elems_final, Grad=Grad,
                                               numcores=theory.numcores,
                                               PC=PC, current_MM_coords=pointchargecoords, MMcharges=pointcharges,
                                               mm_elems=mm_elems_for_qmprogram,
                                               label=label, charge=self.regions_chargemult[j][0],
                                               mult=self.regions_chargemult[j][1])
                else:
                    res = theory.run(current_coords=region_coords_final, elems=region_elems_final, Grad=Grad,
                                     numcores=theory.numcores,
                                     PC=PC, current_MM_coords=pointchargecoords, MMcharges=pointcharges,
                                     mm_elems=mm_elems_for_qmprogram,
                                     label=label, charge=self.regions_chargemult[j][0],
                    ...
                    for i, linkatomindex in enumerate(self.linkatom_indices):
                        ...
                        for theory_grad in [diffgrad]:
                            ...
                            if self.linkatom_forceproj_method == "adv":
                                ...
                                QM1grad_contrib, MM1grad_contrib = linkatom_force_chainrule(Qcoord, Mcoord, Lcoord,
                                                                                            Lgrad)
        ...
        print_time_rel(module_init_time, modulename='ONIOM run', moduleindex=2, currprintlevel=self.printlevel,
                       currthreshold=1)
        if Grad:
