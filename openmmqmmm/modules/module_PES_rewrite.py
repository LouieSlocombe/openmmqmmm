...
from ash.functions.functions_general import ashexit, writestringtofile, BC, blankline, isint, islist, print_time_rel, \
    print_line_with_mainheader, find_between, pygrep
# import ash
from ash.interfaces.interface_ORCA import scfenergygrab, tddftgrab, orbitalgrab, run_orca_plot, grabEOMIPs, \
    check_stability_in_output, ORCATheory, finalenergiesgrab
...
def PhotoElectron(theory=None, fragment=None, method=None, vibrational_option=None, trajectory=None,
    ...
    photo = PhotoElectronClass(theory=theory, fragment=fragment, method=method, vibrational_option=vibrational_option,
                               trajectory=trajectory, numcores=numcores, memory=memory, label=label,
                               Initialstate_charge=Initialstate_charge, Initialstate_mult=Initialstate_mult,
                               Ionizedstate_charge=Ionizedstate_charge, Ionizedstate_mult=Ionizedstate_mult,
                               numionstates=numionstates,
                               initialorbitalfiles=initialorbitalfiles, densities=densities,
                               densgridvalue=densgridvalue,
                               tda=tda, brokensym=brokensym, HSmult=HSmult, atomstoflip=atomstoflip,
                               check_stability=check_stability,
                               deltaSCF_ionize=deltaSCF_ionize, deltaSCF_PMOM=deltaSCF_ionize,
                               CAS_Initial=CAS_Initial, CAS_Final=CAS_Final, no_shakeup=no_shakeup,
...
class PhotoElectronClass:
    def __init__(self, theory=None, fragment=None, method=None, vibrational_option=None, trajectory=None, numcores=1,
                 memory=40000, label=None,
                 Initialstate_charge=None, Initialstate_mult=None,
                 Ionizedstate_charge=None, Ionizedstate_mult=None, numionstates=5,
        ...
        print_line_with_mainheader(
            "PhotoElectron: Calculating PES spectra via TDDFT/CAS/MRCI/EOM/MREOM and Dyson-norm approach")
        ...
        if Initialstate_charge is None or Initialstate_mult is None or Ionizedstate_charge is None or Ionizedstate_mult is None:
            print(
                "Provide charge and spin multiplicity of initial and ionized state: Initialstate_charge, Initialstate_mult, Ionizedstate_charge,Ionizedstate_mult ")
            ashexit()
        if theory is None:
        ...
        if theory.numcores != numcores:
            print(
                f"Warning: Numcores={numcores} provided to PhotoElectron does not match number of cores in theory level.")
        ...
        # Case list provided for ionized state. Could mean multiple spin states: e.g.  Ionizedstate_mult=[5,7]
            ...
            if len(self.Ionizedstate_mult) == 1:
                ...
                self.stateF1 = MolState(charge=self.Ionizedstate_charge, mult=self.Ionizedstate_mult[0],
                                        numionstates=self.numionstates,
                                        label=f'Final_State_mult{self.Ionizedstate_mult[0]}')
                self.Finalstates = [self.stateF1]
                print(BC.OKBLUE, "StateF_1: Charge=", self.Finalstates[0].charge, "Multiplicity",
                      self.Finalstates[0].mult,
                      BC.ENDC)
                ...
                self.stateF1 = MolState(charge=self.Ionizedstate_charge, mult=self.Ionizedstate_mult[0],
                                        numionstates=self.numionstates_A,
                                        label=f'Final_State_mult{self.Ionizedstate_mult[0]}')
                self.stateF2 = MolState(charge=self.Ionizedstate_charge, mult=self.Ionizedstate_mult[1],
                                        numionstates=self.numionstates_B,
                ...
                print(BC.OKBLUE, "StateF_1: Charge=", self.Finalstates[0].charge, "Multiplicity",
                      self.Finalstates[0].mult,
                      BC.ENDC)
                print(BC.OKBLUE, "StateF_2: Charge=", self.Finalstates[1].charge, "Multiplicity",
                      self.Finalstates[1].mult,
                      BC.ENDC)
                print(BC.OKBLUE, "StateF_1: Numionstates=", self.Finalstates[0].numionstates, BC.ENDC)
        ...
        if self.densities == 'SCF':
            print(
                "Will do densities (and difference densities) for Inital-state and Final-state SCF wavefunctions only.")
    ...
    def run_tddft_densities(self, fragment):
        ...
        print("")
        print(BC.OKMAGENTA,
              "Densities option: All . Will do TDDFT-gradient calculation for each TDDFT-state (expensive)", BC.ENDC)
        ...
        for findex, fstate in enumerate(self.Finalstates):
            print(BC.OKGREEN, "Calculating Final State SCF + TDDFT DENSITY CALCULATION. Spin Multiplicity: ",
                  fstate.mult, BC.ENDC)
            shutil.copyfile('../' + 'Final_State_mult' + str(fstate.mult) + '.gbw',
                            'Final_State_mult' + str(fstate.mult) + '.gbw')
            ...
            for tddftstate in range(1, fstate.numionstates):
                ...
                if self.no_shakeup is True:
                    ...
                    if restricted:
                        tddftstring = tddftstring.replace('%tddft',
                ...
                run_orca_plot(orcadir=self.theory.orcadir, filename=self.theory.filename + '.gbw',
                              option='cisspindensity', gridvalue=self.densgridvalue,
                              densityfilename=self.theory.filename + '.cisr')
                os.rename(self.theory.filename + '.spindens.cube',
                          'Final_State_mult' + str(fstate.mult) + 'TDDFTstate_' + str(tddftstate) + '.spindens.cube')
                # Doing eldensity Cubefile for each cisp file and then take difference with Initstate-SCF cubefile
                run_orca_plot(orcadir=self.theory.orcadir, filename=self.theory.filename + '.gbw', option='cisdensity',
                              gridvalue=self.densgridvalue,
                              densityfilename=self.theory.filename + '.cisp')
                os.rename(self.theory.filename + '.eldens.cube',
                          'Final_State_mult' + str(fstate.mult) + 'TDDFTstate_' + str(tddftstate) + '.eldens.cube')

                final_dens = 'Final_State_mult' + str(fstate.mult) + 'TDDFTstate_' + str(tddftstate) + '.eldens.cube'
                cubedict2 = read_cube(final_dens)
                write_cube_diff(self.stateI.cubedict, cubedict2,
                                "Densdiff_SCFInit-TDDFTFinalmult" + str(fstate.mult) + 'TDState' + str(tddftstate))
                print("Wrote Cube file containing density difference between Initial State and Final TDDFT State: ",
                      tddftstate)
        self.theory.cleanup()
        os.chdir('..')
    ...
    def make_diffdensities(self, statetype='MRCI'):
        ...
        os.chdir('Calculated_densities')
        print(
            "Looping over available cube-files and creating difference densities inside directory: Calculated_densities")
        ...
        for fstate in self.Finalstates:
            ...
            if statetype == 'MRCI' or statetype == 'CAS':
                for numstate in range(0, fstate.numionstates):
                    ...
                    write_cube_diff(self.stateI.cubedict, fcubedict,
                                    "Densdiff_Init-Finalmult" + str(fstate.mult) + f'_{statetype}State' + f'{numstate}')
                    print(
                        f"Wrote Cube file containing density difference between Initial State and Final {statetype} State: ",
                        "Densdiff_Init-Finalmult" + str(fstate.mult) + f'_{statetype}State' + f'{numstate}')
            ...
            # For statetype SCF, we have 1 file Init, and for each Final-multiplicity
                ...
                write_cube_diff(self.stateI.cubedict, fcubedict,
                                "Densdiff_Init-Finalmult" + str(fstate.mult) + f'_{statetype}State0')
                print(
                    f"Wrote Cube file containing density difference between Initial State and Final {statetype} State: ",
...
"""
    ...
    def run_MRCI_Initial(self, fragment):
        ...
        casscfblock = self.setup_CASSCF_block(self.MRCI_Initial[0], self.MRCI_Initial[1], 1, self.stateI.mult,
                                              maxiter=200)
        ...
        if self.densities != None:
            run_orca_plot(orcadir=theory.orcadir, filename=f"{theory.filename}.gbw", option='density',
                          specify_density=True,
                          densityfilename=theory.filename + f'.state_0_block_0.el.tmp', gridvalue=self.densgridvalue,
                          individual_file=True)
            shutil.copyfile(theory.filename + '.eldens.cube',
                            './Calculated_densities/' + f"{self.stateI.label}.eldens.cube")
            ...
            if self.stateI.mult > 1:
                run_orca_plot(orcadir=self.theory.orcadir, filename=f"{self.theory.filename}.gbw", option='spindensity',
                              specify_density=True,
                              densityfilename=theory.filename + f'.state_0_block_0.spin.tmp',
                              gridvalue=self.densgridvalue, individual_file=True)
                shutil.copyfile(theory.filename + '.spindens.cube',
                                './Calculated_densities/' + f"{self.stateI.label}.spindens.cube")
    ...
    def run_MRCI_Final(self, fragment):
        ...
        casscfblock = self.setup_CASSCF_block(self.MRCI_Final[0], self.MRCI_Final[1], numionstates_string, MRCI_mults,
                                              maxiter=200)
        ...
        print(BC.OKGREEN, "Calculating Final State MRCI Spin Multiplicities: ", [f.mult for f in self.Finalstates],
              BC.ENDC)
        ...
        # RUNNING JOB
        ash.Singlepoint(fragment=fragment, theory=theory, charge=self.Finalstates[0].charge,
                        mult=self.Finalstates[0].mult)
            ...
            for fstate in self.Finalstates:
                ...
                for numstate in range(0, fstate.numionstates):
                    run_orca_plot(orcadir=theory.orcadir, filename=f"{theory.filename}.gbw", option='density',
                                  ...
                                  gridvalue=self.densgridvalue, individual_file=True)
                    shutil.copyfile(theory.filename + '.eldens.cube',
                                    './Calculated_densities/' + f"{fstate.label}_state{numstate}.eldens.cube")
                    os.remove(theory.filename + f'.state_{numstate}_block_{numblock}.el.tmp')
                    # Only spin density in CAS/MRCI if spinmult >1
                    if fstate.mult > 1:
                        run_orca_plot(orcadir=theory.orcadir, filename=f"{theory.filename}.gbw", option='spindensity',
                                      specify_density=True,
                        ...
                        shutil.copyfile(theory.filename + '.spindens.cube',
                                        './Calculated_densities/' + f"{fstate.label}_state{numstate}.spindens.cube")
...
"""
        ...
        casscfblock = self.setup_CASSCF_block(self.CAS_Initial[0], self.CAS_Initial[1], 1, self.stateI.mult,
                                              maxiter=200,
                                              extraline=f"printwf det\nci TPrintwf {self.tprintwfvalue} end\n{nevpt2_line}")
        ...
        if self.densities != None:
            ...
            run_orca_plot(orcadir=theory.orcadir, filename=f"{theory.filename}.gbw", option='density',
                          specify_density=True,
                          densityfilename=theory.filename + f'.cas.mult.{self.stateI.mult}.root.0.scfp',
                          gridvalue=self.densgridvalue)
            shutil.copyfile(theory.filename + '.eldens.cube',
                            './Calculated_densities/' + f"{self.stateI.label}.eldens.cube")
            # Only spin density in CAS/MRCI if spinmult >1
            if self.stateI.mult > 1:
                run_orca_plot(orcadir=theory.orcadir, filename=f"{self.theory.filename}.gbw", option='spindensity',
                              specify_density=True,
                              densityfilename=theory.filename + f'.cas.mult.{self.stateI.mult}.root.0.scfp',
                              gridvalue=self.densgridvalue)
                shutil.copyfile(theory.filename + '.spindens.cube',
...
"""
        ...
        casscfblock = self.setup_CASSCF_block(self.CAS_Final[0], self.CAS_Final[1], numionstates_string, CAS_mults,
                                              maxiter=200,
                                              extraline=f"printwf det\nci TPrintwf {self.tprintwfvalue} end\n{nevpt2_line}")
        ...
        print(BC.OKGREEN, "Calculating Final State CASSCF Spin Multiplicities: ", [f.mult for f in self.Finalstates],
              BC.ENDC)
        ...
        ash.Singlepoint(fragment=fragment, theory=theory2, charge=self.Finalstates[0].charge,
                        mult=self.Finalstates[0].mult)
        ...
        if self.densities != None:
            ...
            for fstate in self.Finalstates:
                ...
                print("Now creating density file for each root")
                for numstate in range(0, fstate.numionstates):
                    run_orca_plot(orcadir=theory2.orcadir, filename=f"{theory2.filename}.gbw", option='density',
                                  specify_density=True,
                    ...
                    # Only spin density in CAS/MRCI if spinmult >1
                    if fstate.mult > 1:
                        run_orca_plot(orcadir=theory2.orcadir, filename=f"{theory2.filename}.gbw", option='spindensity',
                                      specify_density=True,
                                      densityfilename=theory2.filename + f'.cas.mult.{fstate.mult}.root.{numstate}.scfr',
                                      gridvalue=self.densgridvalue)
                        # Copy density files for each final state over to Calculated_densities
                        shutil.copyfile(theory2.filename + '.spindens.cube',
                                        './Calculated_densities/' + f"{fstate.label}_state{numstate}.spindens.cube")
                ...
                if self.densities == 'SCF' or self.densities == 'All':
                    ...
                    if self.stateI.hftyp == "UHF":
                        run_orca_plot(orcadir=theory.orcadir, filename=f"{theory.filename}.gbw", option='spindensity',
                                      gridvalue=self.densgridvalue)
                        # Move into Calculated_densities dir
                        shutil.move(f"{theory.filename}.spindens.cube",
                                    'Calculated_densities/' + f"{label}.spindens.cube")
        return IPs_all, Ionstates_energies_all
    ...
    def run_TDDFT(self, fragment):
        ...
        for findex, fstate in enumerate(self.Finalstates):
            ...
            if self.initialorbitalfiles is not None:
                ...
                if len(self.initialorbitalfiles) > 1:
                    print("Will use file {} as guess GBW file for this Final state.".format(
                        self.initialorbitalfiles[findex + 1]))
            ...
            if len(fstate.TDtransitionenergies) != fstate.numionstates - 1:
                print(
                    f"Warning: TDDFT calculated fewer states ({len(fstate.TDtransitionenergies)}) than requested ({fstate.numionstates})")
            ...
            if self.densities == 'SCF' or self.densities == 'All':
                ...
                run_orca_plot(orcadir=theory.orcadir, filename=f"{theory.filename}.gbw", option='density',
                              gridvalue=self.densgridvalue)
                # Move into Calculated_densities dir
                shutil.move(f"{theory.filename}.eldens.cube",
                            'Calculated_densities/' + f"{fstate.label}_state0.eldens.cube")
                ...
                if fstate.hftyp == "UHF":
                    ...
                    # Move into Calculated_densities dir
                    shutil.move(f"{theory.filename}.spindens.cube",
                                'Calculated_densities/' + f"{fstate.label}_state0.spindens.cube")
        ...
        blankline()
        print(
            "All SCF and TDDFT calculations are now done (unless Densities=All, then we will calculate TDDFT densities at the end)!")
        ...
        for fstate in self.Finalstates:
            ...
            for e in fstate.TDtransitionenergies:
                ...
                fstate.IPs.append(
                    (e / ash.constants.hartoeV + fstate.energy - self.stateI.energy) * ash.constants.hartoeV)
            print("")
    ...
    def run_SF_TDDFT(self, fragment):
        ...
        if self.tda == False:
            tddftstring = "%tddft\n" + "tda false\n" + "nroots " + str(
                highest_mult_fstate.numionstates - 1) + '\n' + "maxdim 25\n" + "maxiter 15\n" + "end\n" + "\n"
        else:
            tddftstring = "%tddft\n" + "tda true\n" + "nroots " + str(
                highest_mult_fstate.numionstates - 1) + '\n' + "maxdim 25\n" + "end\n" + "\n"
        theory.extraline = tddftstring
        ...
        ash.Singlepoint(fragment=fragment, theory=theory, charge=highest_mult_fstate.charge,
                        mult=highest_mult_fstate.mult)
        stability = check_stability_in_output(theory.filename + '.out')
        if stability is False and self.check_stability is True:
            print("PES: Unstable final state. Exiting...")
            ashexit()
        ...
        # Final state orbitals for MO-DOSplot
        highest_mult_fstate.occorbs_alpha, highest_mult_fstate.occorbs_beta, highest_mult_fstate.hftyp = orbitalgrab(
            theory.filename + '.out')
        ...
        if self.densities == 'SCF' or self.densities == 'All':
            ...
            # Move into Calculated_densities dir
            shutil.move(f"{theory.filename}.eldens.cube",
                        'Calculated_densities/' + f"{highest_mult_fstate.label}_state0.eldens.cube")
            # f"{fstate.label}_state{numstate}.eldens.cube"
            # Spin density (only if UHF). Otherwise orca_plot gets confused (takes difference between alpha-density and nothing)
            if highest_mult_fstate.hftyp == "UHF":
                ...
                # Move into Calculated_densities dir
                shutil.move(f"{theory.filename}.spindens.cube",
                            'Calculated_densities/' + f"{highest_mult_fstate.label}_state0.spindens.cube")
        ...
        print(BC.OKGREEN,
              f"\nCalculating Lower-spinmultiplicity (mult: {lowest_mult_fstate.mult}) via spinFlip-TDDFT from higher-mult SCF(mult: {highest_mult_fstate}).",
              BC.ENDC)
        # Setting TDA/TDDFT states for each spin multiplicity
        if self.tda == False:
            tddftstring = "%tddft\n" + "sf true\n" + "tda false\n" + "nroots " + str(
                lowest_mult_fstate.numionstates) + '\n' + "maxdim 25\n" + "maxiter 15\n" + "end\n" + "\n"
        else:
            tddftstring = "%tddft\n" + "sf true\n" + "tda true\n" + "nroots " + str(
                lowest_mult_fstate.numionstates) + '\n' + "maxdim 25\n" + "end\n" + "\n"
        theory.extraline = tddftstring
        # Run Spin-Flip TDDFT, using highest-spin multiplicity
        ash.Singlepoint(fragment=fragment, theory=theory, charge=lowest_mult_fstate.charge,
                        mult=highest_mult_fstate.mult)
        stability = check_stability_in_output(theory.filename + '.out')
        if stability is False and self.check_stability is True:
            print("PES: Unstable final state. Exiting...")
        ...
        # Final state orbitals for MO-DOSplot
        lowest_mult_fstate.occorbs_alpha, lowest_mult_fstate.occorbs_beta, lowest_mult_fstate.hftyp = orbitalgrab(
            theory.filename + '.out')
        ...
        # Spin-flipped IPs
        SF_IPs = sorted([GS_highest_mult_IP + SF_transenergy for SF_transenergy in
                         SF_TDtransitionenergies])  # Converting SF transition energies into IPs and sorting
        print("SF_IPs:", SF_IPs)
        ...
        print(BC.OKBLUE,
              f"TDDFT transition energies (eV) for FinalState (mult: {highest_mult_fstate.mult}) : {highest_mult_fstate.TDtransitionenergies}\n",
              BC.ENDC, )
        print(BC.OKBLUE,
              f"Spin-Flip TDDFT transition energies (eV) for FinalState (mult: {lowest_mult_fstate.mult}) : {lowest_mult_fstate.TDtransitionenergies}\n",
              BC.ENDC, )
        ...
        for e in highest_mult_fstate.TDtransitionenergies:
            ...
            highest_mult_fstate.IPs.append(
                (e / ash.constants.hartoeV + highest_mult_fstate.energy - self.stateI.energy) * ash.constants.hartoeV)
...
"""
            ...
            init_EOM = ash.Singlepoint(fragment=fragment, theory=theory, charge=self.Initialstate_charge,
                                       mult=self.Initialstate_mult)
        ...
        print(BC.WARNING,
              "WARNING: Dyson norms not calculated for IP-EOM-CCSD. Instead using dominant singles amplitudes as an approximation",
              BC.ENDC)
    ...
    def run_SCF_InitState(self, fragment, theory):
        ...
        self.InitSCF = ash.Singlepoint(fragment=fragment, theory=theory, charge=self.Initialstate_charge,
                                       mult=self.Initialstate_mult)
    ...
    def mo_spectrum(self):
        # MO IP spectrum:
        self.stk_alpha, self.stk_beta = modosplot(self.stateI.occorbs_alpha, self.stateI.occorbs_beta,
                                                  self.stateI.hftyp)
        ...
        if self.method == 'CASSCF' or self.method == 'CASCI' or self.method == 'MRCI' or self.method == 'MREOM' or self.method == 'NEVPT2' or self.method == 'NEVPT2-F12':
            ...
            if self.method == 'CASSCF':
                if self.CASCI_Final is True:
                    ...
                    stype = 'SCF'
            print("{:>6} {:>7} {:^20} {:8} {:10} {:>7}".format("State no.", "Mult", "TotalE (Eh)", "IE (eV)",
                                                               "Dyson-norm", "State-type"))
            ...
            print("{:>6} {:>7} {:^20} {:8} {:10} {:>7} {:>15}".format("State no.", "Mult", "TotalE (Eh)", "IE (eV)",
                                                                      "Dyson-norm", "State-type", "TDDFT Exc.E. (eV)"))
            fstate = self.Finalstates[0]
            for i, (E, IE, dys, statelabel, TDtransenergy, spinmult) in enumerate(
                    zip(Finalionstates, IPs, dysonnorms, statelabels, tdtransitions, spinmults)):
                print(
                    f"{i:>6d} {spinmult:>7d} {E:20.11f} {IE:>10.3f} {dys:>10.5f} {statelabel:>10} {TDtransenergy:>17.3f}")
            ...
            statelabels = ['SCF'] + [tdtype] * (highest_mult_fstate.numionstates - 1) + [
                sftdtype] * lowest_mult_fstate.numionstates
            tdtransitions = [0.0] + highest_mult_fstate.TDtransitionenergies + lowest_mult_fstate.TDtransitionenergies
            spinmults = [highest_mult_fstate.mult] * highest_mult_fstate.numionstates + [
                lowest_mult_fstate.mult] * lowest_mult_fstate.numionstates
            ...
            for i, (E, IE, dys, statelabel, TDtransenergy, spinmult) in enumerate(
                    zip(Finalionstates, IPs, dysonnorms, statelabels, tdtransitions, spinmults)):
                print("{:>6d} {:>7d} {:20.11f} {:>10.3f} {:>10.5f} {:>10} {:>17.3f}".format(i, spinmult, E, IE, dys,
                                                                                            statelabel, TDtransenergy))
    ...
    def TDDFT_dets_prep(self):
        ...
        for fstate in self.Finalstates:
            # mults = [stateFmult]
            mults = [fstate.mult]
            det_final = get_dets_from_cis(fstate.outfile, fstate.cisfile, fstate.restricted, mults, fstate.charge,
                                          fstate.mult,
                                          self.totnuccharge, [1, fstate.numionstates], statestoskip, self.no_tda,
                                          self.frozencore, self.wfthres)
            # Printing to file
    ...
    def run(self):
        ...
        self.Finalionstates = [];
        self.finaldysonnorms = [];
        self.FinalTDtransitionenergies = []
        ...
        if self.method == 'TDDFT':
            ...
            for geonum, fragment in enumerate(fragments):
                ...
                for fstate in self.Finalstates:
                    ...
                    totnumorbitals, numocc_alpha, numocc_beta, restricted = get_orb_info_from_dict(
                        final_state_data_dict)
                    # Write Final-state MOs to disk in wfoverlap format
                    create_wfoverlap_MO_file(final_state_data_dict, "mos_final-mult" + str(fstate.mult),
                                             mo_threshold=1e-12, frozencore=0)
...
# TODO: Rewrite this
def get_dets_from_cis(logfile, cisfilename, restr, mults, gscharge, gsmult, totnuccharge, nstates_to_extract,
                      nstates_to_skip, no_tda, frozencore, wfthres):
            ...
            for key in dets2:
                ...
                if restr:
                    ...
                    if any([key[i] != 1 for i in range(frozencore)]):
                        problem = True
                    if any([key[i] != 2 for i in
                            range(nocc_A + nvir_A + frozencore, nocc_A + nvir_A + 2 * frozencore)]):
                        problem = True
                if problem:
...
def run_wfoverlap(wfoverlapinput, path_wfoverlap, memory, numcores):
    ...
    with open("wfovl.out", 'w') as f:
        process = sp.run([path_wfoverlap, '-m', str(memory), '-f', 'wfovl.inp'], env=os.environ, check=True, stdout=f,
            ...
            if detgrab is True:
                ...
                if '[' in line and 'CFG' in line:
                    ...
                    if '1' not in cfg:
                        ...
                        print(
                            "WARNING: Det info is probably missing (not printed). Taking CFG and weight and converting to determinant")
                        # print("line:", line)
            ...
            if detgrab is True:
                ...
                if '[' in line and 'CFG' not in line:
                    ...
                    if det_elcount != tot_num_electrons:
                        print("det_tuple:", det_tuple)
                        print(
                            f"Problem. Determinant electron-count({det_elcount}) does not add up to correct electron-number({tot_num_electrons})")
                        ashexit()
                    det_ms = check_spin_of_tuple(det_tuple)
                    if det_ms != (mult - 1) / 2:
                        print("det_tuple:", det_tuple)
                        print(
                            f"Problem. Determinant spin or M_S value ({det_ms}) does not add up to correct spin (mult:{mult} i.e. M_S: {(mult - 1) / 2:})")
                        ashexit()
