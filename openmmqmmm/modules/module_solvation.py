...
def solvshell(orcadir='', NumCores='', calctype='', orcasimpleinput_LL='',
    ...
    snapshotinpfiles = ash.functions.functions_solv.create_AB_inputfiles_ORCA(solute_atoms, solvent_atoms, solvsphere,
                                                                              solvsphere.snapshots,
                                                                              orcasimpleinput_LL, orcablockinput_LL,
                                                                              solventunitcharges, identifiername)
    if calctype == "redox":
    ...
    ###################################
    blankline()
    AllsnapsABenergy, AsnapsABenergy, BsnapsABenergy = ash.functions.functions_solv.grab_energies_output_ORCA(
        snapshotinpfiles)
    blankline()
    ...
    if LongRangePolarization == True:
        ...
        results = pool.map(LRPolsnapshotcalc, [[snapshot, solvsphere, psi4dict, psi4_functional, pot_option,
                                                LRPolRegion1, LRPolRegion2, NumCoresPsi4, LRPolQMRegion, psi4memory,
                                                psi4runmode]
...
def LRPolsnapshotcalc(args):
    ...
    PEsolvshell_LR1 = ash.functions.functions_solv.get_solvshell(solvsphere, snap_frag.elems, snap_frag.coords,
                                                                 LRPolRegion1, qmatoms_LR1_elems,
                                                                 qmatoms_LR1_coords,
                                                                 ash.settings_solvation.scale,
                                                                 ash.settings_solvation.tol)
    ...
    PEsolvshell_LR2 = ash.functions.functions_solv.get_solvshell(solvsphere, snap_frag.elems, snap_frag.coords,
                                                                 LRPolRegion2, solute_elems,
                                                                 solute_coords,
                                                                 ash.settings_solvation.scale,
                                                                 ash.settings_solvation.tol)
    qm_solvshell_LR2 = ash.functions.functions_solv.get_solvshell(solvsphere, snap_frag.elems, snap_frag.coords,
                                                                  LRPolQMRegion, solute_elems,
                                                                  solute_coords,
                                                                  ash.settings_solvation.scale,
                                                                  ash.settings_solvation.tol)
    ...
    if len(qmatoms_LR2) + len(peatoms_LR2) + len(mmatoms_LR2) != len(solvsphere.allatoms):
        print("QM + MM + PE atoms ({})not equal to total numatoms({})".format(
            len(qmatoms_LR2) + len(peatoms_LR2) + len(mmatoms_LR2),
    ...
    PolEmbedEnergyA_LR1 = PolEmbed_SP_A_LR1.run(potfile=snapshot + 'System_LR1.pot', numcores=NumCoresPsi4,
                                                restart=False)
    ...
    PolEmbedEnergyB_LR1 = PolEmbed_SP_B_LR1.run(potfile=snapshot + 'System_LR1.pot', numcores=NumCoresPsi4,
                                                restart=True)
    ...
    PolEmbedEnergyA_LR2 = PolEmbed_SP_A_LR2.run(potfile=snapshot + 'System_LR2.pot', numcores=NumCoresPsi4,
                                                restart=False)
    ...
    PolEmbedEnergyB_LR2 = PolEmbed_SP_B_LR2.run(potfile=snapshot + 'System_LR2.pot', numcores=NumCoresPsi4,
                                                restart=True)
