...
def Singlepoint_reaction(theory=None, reaction=None, moreadfiles=None):
    ...
    if isinstance(theory, ash.ORCA_CC_CBS_Theory):
        ...
        if 'E_SCF_CBS' in componentsdict:
            ...
            deltaSCF = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                      list_of_energies=scf_parts, unit=unit, label='ΔSCF')[0]
        ...
        if 'E_corrCCSD_CBS' in componentsdict:
            ...
            delta_CCSDcorr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                            list_of_energies=ccsd_parts, unit=unit, label='ΔCCSD')[0]
        ...
        if 'E_corrCCT_CBS' in componentsdict:
            ...
            delta_Tcorr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                         list_of_energies=triples_parts, unit=unit, label='Δ(T)')[0]
        ...
        if 'E_corr_CBS' in componentsdict:
            ...
            delta_CC_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                           list_of_energies=valencecorr_parts, unit=unit, label='ΔCCSD+Δ(T) corr')[0]
            finaldict['delta_CC_corr'] = delta_CC_corr
        if 'E_SO' in componentsdict:
            SO_parts = [d['E_SO'] for d in list_of_componentsdicts]
            delta_SO_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                           list_of_energies=SO_parts, unit=unit, label='ΔSO')[0]
            finaldict['delta_SO_corr'] = delta_SO_corr
        if 'E_HOCC' in componentsdict:
            HOCC_parts = [d['E_HOCC'] for d in list_of_componentsdicts]
            delta_HOCC_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                             list_of_energies=HOCC_parts, unit=unit, label='ΔHOCC')[0]
            finaldict['delta_HOCC_corr'] = delta_HOCC_corr
        if 'E_DBOC' in componentsdict:
            DBOC_parts = [d['E_DBOC'] for d in list_of_componentsdicts]
            delta_DBOC_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                             list_of_energies=DBOC_parts, unit=unit, label='ΔDBOC')[0]
        ...
        if 'E_corecorr_and_SR' in componentsdict:
            ...
            delta_CVSR_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                             list_of_energies=CV_SR_parts, unit=unit, label='ΔCV+SR')[0]
        ...
        if 'T1energycorr' in componentsdict:
            ...
            delta_T1_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                           list_of_energies=T1corr_parts, unit=unit, label='ΔΔT1corr')[0]
        ...
        if 'E_FCIcorrection' in componentsdict:
            ...
            delta_FCI_corr = ReactionEnergy(stoichiometry=reaction.stoichiometry, list_of_fragments=reaction.fragments,
                                            list_of_energies=fcicorr_parts, unit=unit, label='ΔFCIcorr')[0]
