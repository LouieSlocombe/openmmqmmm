...
from ash.functions.functions_general import BC, print_time_rel, print_line_with_mainheader, pygrep, ashexit, \
    n_max_values
...
def confsampler_protocol(fragment=None, crestdir=None, xtbmethod='GFN2-xTB', MLtheory=None,
    ...
    list_conformer_frags, xtb_energies = ash.interfaces.interface_crest.call_crest(fragment=fragment,
                                                                                   xtbmethod=xtbmethod,
                                                                                   crestdir=crestdir,
                                                                                   charge=charge, mult=mult,
                                                                                   numcores=numcores,
                                                                                   extraoptions=crestoptions)
    ...
    for index, conformer in enumerate(list_conformer_frags):
        ...
        ash.interfaces.interface_geometric_new.geomeTRICOptimizer(fragment=conformer, theory=MLtheory,
                                                                  coordsystem='tric', charge=charge, mult=mult,
...
def thermochemprotocol_single(fragment=None, Opt_theory=None, SP_theory=None, numcores=None, memory=5000,
    ...
    if fragment.numatoms != 1:
        if Opt_theory != None:
            ...
            # DFT-opt
            ash.interfaces.interface_geometric_new.geomeTRICOptimizer(theory=Opt_theory, fragment=fragment,
                                                                      charge=charge, mult=mult)
            print("-------------------------------------------------------------------------")
            ...
            if analyticHessian is True:
                ...
                freq_result = ash.NumFreq(fragment=fragment, theory=Opt_theory, npoint=2, runmode='serial',
                                          charge=charge, mult=mult)
                thermochem = freq_result.thermochemistry
        else:
...
def thermochemprotocol_reaction(Opt_theory=None, SP_theory=None, reaction=None, fraglist=None, stoichiometry=None,
                                numcores=1, memory=5000,
    ...
    for i, frag in enumerate(fraglist):
        print(
            f"Fragment {i} Formula: {frag.prettyformula}  Label: {frag.label} Charge: {frag.charge} Mult: {frag.mult}")
    ...
    for species in fraglist:
        ...
        FinalE, componentsdict, thermochem = thermochemprotocol_single(fragment=species, Opt_theory=Opt_theory,
                                                                       SP_theory=SP_theory, numcores=numcores,
                                                                       memory=memory,
                                                                       analyticHessian=analyticHessian, temp=temp,
                                                                       pressure=pressure, charge=species.charge,
                                                                       mult=species.mult)
    ...
    print("-" * 80)
    deltaE = ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=FinalEnergies_el,
                            unit=unit, label='Total ΔE_el')[0]
    deltaE_0 = \
        ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=FinalEnergies_zpve,
                       unit=unit, label='Total Δ(E+ZPVE)')[0]
    deltaH = \
        ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=FinalEnthalpies,
                       unit=unit,
                       label='Total ΔH(T={}'.format(temp))[0]
    deltaG = ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=FinalFreeEnergies,
                            unit=unit, label='Total ΔG(T={}'.format(temp))[0]
    print("-" * 80)
    print("Individual contributions")
    # Print individual contributions if available
    # ZPVE, Hcorr, gcorr
    deltaZPVE = \
        ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=ZPVE_Energies,
                       unit=unit,
                       label='ΔZPVE')[0]
    deltaHcorr = \
        ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=Hcorr_Energies,
                       unit=unit,
                       label='ΔHcorr')[0]
    deltaGcorr = \
        ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=Gcorr_Energies,
                       unit=unit,
                       label='ΔGcorr')[0]
    print("-" * 80)
    finaldict = {'deltaE': deltaE, 'deltaE_0': deltaE_0, 'deltaH': deltaH, 'deltaG': deltaG, 'deltaZPVE': deltaZPVE,
                 'deltaHcorr': deltaHcorr, 'deltaGcorr': deltaGcorr}
    # Contributions to CCSD(T) energies
    if 'E_SCF_CBS' in componentsdict:
        scf_parts = [d['E_SCF_CBS'] for d in list_of_dicts]
        deltaSCF = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=scf_parts,
                           unit=unit,
                           label='ΔSCF')[0]
    ...
    if 'E_corrCCSD_CBS' in componentsdict:
        ...
        delta_CCSDcorr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=ccsd_parts,
                           unit=unit,
                           label='ΔCCSD')[0]
    ...
    if 'E_corrCCT_CBS' in componentsdict:
        ...
        delta_Tcorr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=triples_parts,
                           unit=unit, label='Δ(T)')[0]
        finaldict['delta_Tcorr'] = delta_Tcorr
    if 'E_corr_CBS' in componentsdict:
        valencecorr_parts = [d['E_corr_CBS'] for d in list_of_dicts]
        delta_CC_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=valencecorr_parts,
                           unit=unit, label='ΔCCSD+Δ(T) corr')[0]
        finaldict['delta_CC_corr'] = delta_CC_corr
    if 'E_SO' in componentsdict:
        SO_parts = [d['E_SO'] for d in list_of_dicts]
        delta_SO_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=SO_parts,
                           unit=unit,
                           label='ΔSO')[0]
    ...
    if 'E_HOCC' in componentsdict:
        ...
        delta_HOCC_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=HOCC_parts,
                           unit=unit,
                           label='ΔHOCC')[0]
    ...
    if 'E_DBOC' in componentsdict:
        ...
        delta_DBOC_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=DBOC_parts,
                           unit=unit,
                           label='ΔDBOC')[0]
    ...
    if 'E_corecorr_and_SR' in componentsdict:
        ...
        delta_CVSR_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=CV_SR_parts,
                           unit=unit,
                           label='ΔCV+SR')[0]
    ...
    if 'T1energycorr' in componentsdict:
        ...
        delta_T1_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=T1corr_parts,
                           unit=unit, label='ΔΔT1corr')[0]
    ...
    if 'E_FCIcorrection' in componentsdict:
        ...
        delta_FCI_corr = \
            ReactionEnergy(stoichiometry=stoichiometry, list_of_fragments=fraglist, list_of_energies=fcicorr_parts,
                           unit=unit, label='ΔFCIcorr')[0]
...
def auto_active_space(fragment=None, orcadir=None, basis="def2-SVP", scalar_rel=None, charge=None, mult=None,
                      initial_orbitals='MP2', functional='TPSS', smeartemp=5000, tgen=1e-1,
                      selection_thresholds=[1.999, 0.001],
    ...
    if initial_orbitals == 'MP2':
        ...
        ORCAcalc_1 = ash.ORCATheory(orcadir=orcadir, orcasimpleinput=orcasimpleinput, orcablocks=orcablocks,
                                    numcores=numcores)
        ash.Singlepoint(theory=ORCAcalc_1, fragment=fragment, charge=charge, mult=mult)
        step1occupations, qroenergies = ash.interfaces.interface_ORCA.QRO_occ_energies_grab(
            ORCAcalc_1.filename + '.out')
    ...
    numelectrons, numorbitals = ash.functions.functions_elstructure.select_space_from_occupations(step1occupations,
    ...
    minimal_CAS = ash.functions.functions_elstructure.select_space_from_occupations(ICEnatoccupations,
                                                                                    selection_thresholds=[1.95, 0.05])
    medium1_CAS = ash.functions.functions_elstructure.select_space_from_occupations(ICEnatoccupations,
                                                                                    selection_thresholds=[1.98, 0.02])
    medium2_CAS = ash.functions.functions_elstructure.select_space_from_occupations(ICEnatoccupations,
                                                                                    selection_thresholds=[1.985, 0.015])
    medium3_CAS = ash.functions.functions_elstructure.select_space_from_occupations(ICEnatoccupations,
                                                                                    selection_thresholds=[1.99, 0.01])
    medium4_CAS = ash.functions.functions_elstructure.select_space_from_occupations(ICEnatoccupations,
                                                                                    selection_thresholds=[1.992, 0.008])
    large_CAS = ash.functions.functions_elstructure.select_space_from_occupations(ICEnatoccupations,
                                                                                  selection_thresholds=[1.995, 0.005])

    spaces_dict = {"minimal_CAS": minimal_CAS, "medium1_CAS": medium1_CAS, "medium2_CAS": medium2_CAS,
                   "medium3_CAS": medium3_CAS, "medium4_CAS": medium4_CAS, "large_CAS": large_CAS}
...
def calc_xyzfiles(xyzdir=None, theory=None, HL_theory=None, Opt=False, Freq=False, charge=None, mult=None,
                  xtb_preopt=False):
    ...
    if os.path.isdir(xyzdir) is False:
        print(
            "XYZ directory does not exist. Check that dirname is complete, has been copied to scratch. You may have to use full path to dir")
        ashexit()
    ...
    if charge == None or mult == None:
        print(
            "Charge/mult options are None. This means that XYZ-files must have charge/mult information in their header\n\n")
        readchargemult = True
    else:
    ...
    for fragment in fragments:
        ...
        if Opt is True:
            if xtb_preopt is True:
                print(
                    "xTB Pre-optimization is active. Will first call xTB directly to do pre-optimization before actual optimization!")
            ...
            optresult = ash.interfaces.interface_geometric_new.geomeTRICOptimizer(theory=theory, fragment=fragment,
                                                                                  coordsystem='tric',
            ...
            # In case of atoms
            if fragment.numatoms > 1:
                os.rename("Fragment-optimized.xyz",
                          os.path.splitext(fragment.label)[0] + filenamestring_suffix + ".xyz")
            ...
            if Freq is True:
                ...
                thermochem = ash.NumFreq(fragment=fragment, theory=theory, npoint=2, displacement=0.005,
                                         numcores=theory.numcores, runmode='serial', charge=fragment.charge,
                                         mult=fragment.mult)
                theory.cleanup()
            if Highlevel is True:
                print("Performing Highlevel on Optimized fragment.")
                hl_result = ash.Singlepoint(theory=HL_theory, fragment=fragment, charge=fragment.charge,
                                            mult=fragment.mult)
...
def Reaction_Highlevel_Analysis(reaction=None, numcores=1, memory=7000, plot=True,
    ...
    if def2_family is True:
        ...
        for cardinal in CCSDT_def2_bases_proj.cardinals:
            ...
            # Define theory level
            cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=[cardinal], basisfamily="def2", DLPNO=DLPNO,
                                    numcores=numcores, memory=memory)
            ...
            reaction_energy, unused = ReactionEnergy(stoichiometry=stoichiometry, list_of_energies=energies,
                                                     unit=energy_unit, label=reactionlabel, silent=False)
    ...
    if cc_family is True:
        ...
        for cardinal in CCSDT_cc_bases_proj.cardinals:
            ...
            # Define theory level
            cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=[cardinal], basisfamily="cc", DLPNO=DLPNO,
                                    numcores=numcores, memory=memory)
            # Single-point calcs on all fragments
            ...
            # Store reaction energy in dict
            reaction_energy, unused = ReactionEnergy(stoichiometry=stoichiometry, list_of_energies=energies,
                                                     unit='kcal/mol', label=reactionlabel, silent=False)
    ...
    if aug_cc_family is True:
        ...
        for cardinal in CCSDT_aug_cc_bases_proj.cardinals:
            ...
            # Define theory level
            cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=[cardinal], basisfamily="aug-cc", DLPNO=DLPNO,
                                    numcores=numcores, memory=memory)
            # Single-point calcs on all fragments
    ...
    if F12_family is True:
        ...
        for cardinal in CCSDTF12_bases_proj.cardinals:
            ...
            # Define theory level
            cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=[cardinal], basisfamily="cc-f12", F12=True,
                                    DLPNO=DLPNO, numcores=numcores, memory=memory)
            # Single-point calcs on all fragments
    ...
    if extrapolation is True:
        ...
        for cardinals in CCSDTextrap_proj.cardinals:
            ...
            # Define theory level
            cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=cardinals, basisfamily="cc", DLPNO=DLPNO,
                                    numcores=numcores, memory=memory)
            # Single-point calcs on all fragments
    ...
    if extrapolation is True:
        if aug_cc_family is True:
            ...
            for cardinals in CCSDTextrapaugcc_proj.cardinals:
                ...
                # Define theory level
                cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=cardinals, basisfamily="aug-cc",
                                        DLPNO=DLPNO, numcores=numcores, memory=memory)
                # Single-point calcs on all fragments
    ...
    if extrapolation is True:
        ...
        for cardinals in CCSDTextrapdef2_proj.cardinals:
            ...
            # Define theory level
            cc = ORCA_CC_CBS_Theory(elements=elements_involved, cardinals=cardinals, basisfamily="def2", DLPNO=DLPNO,
                                    numcores=numcores, memory=memory)
            # Single-point calcs on all fragments
    ...
    if plot is True:
        ...
        for species in specieslist:
            ...
            eplot = ASH_plot('{} energy plot'.format(specieslabel), num_subplots=1, x_axislabel="Cardinal",
                             y_axislabel='Energy (Eh)', title='{} Energy'.format(specieslabel))
            print("eplot:", eplot)
            ...
            if cc_family is True:
                eplot.addseries(0, x_list=CCSDT_cc_bases_proj.cardinals,
                                y_list=CCSDT_cc_bases_proj.species_energies_dict[specieslabel], label='cc-pVnZ',
                                color='blue')
            if aug_cc_family is True:
                eplot.addseries(0, x_list=CCSDT_aug_cc_bases_proj.cardinals,
                                y_list=CCSDT_aug_cc_bases_proj.species_energies_dict[specieslabel], label='aug-cc-pVnZ',
                                color='lightblue')
            if def2_family is True:
                eplot.addseries(0, x_list=CCSDT_def2_bases_proj.cardinals,
                                y_list=CCSDT_def2_bases_proj.species_energies_dict[specieslabel], label='def2-nVP',
                                color='red')
            if F12_family is True:
                eplot.addseries(0, x_list=CCSDTF12_bases_proj.cardinals,
                                y_list=CCSDTF12_bases_proj.species_energies_dict[specieslabel], label='cc-pVnZ-F12',
                                color='purple')
            if extrapolation is True:
                eplot.addseries(0, x_list=[2.5], y_list=[CCSDTextrap_proj.species_energies_dict[specieslabel][0]],
                                label='CBS-cc-23', line=False, marker='x', color='gray')
                eplot.addseries(0, x_list=[3.5], y_list=[CCSDTextrap_proj.species_energies_dict[specieslabel][1]],
                                label='CBS-cc-34', line=False, marker='x', color='green')
                if aug_cc_family is True:
                    eplot.addseries(0, x_list=[2.5],
                                    y_list=[CCSDTextrapaugcc_proj.species_energies_dict[specieslabel][0]],
                                    label='CBS-aug-cc-23', line=False, marker='x', color='brown')
                    eplot.addseries(0, x_list=[3.5],
                                    y_list=[CCSDTextrapaugcc_proj.species_energies_dict[specieslabel][1]],
                                    label='CBS-aug-cc-34', line=False, marker='x', color='lightgreen')
                if highest_cardinal > 4:
                    eplot.addseries(0, x_list=[4.5], y_list=[CCSDTextrap_proj.species_energies_dict[specieslabel][2]],
                                    label='CBS-cc-45', line=False, marker='x', color='black')
                    if aug_cc_family is True:
                        eplot.addseries(0, x_list=[4.5],
                                        y_list=[CCSDTextrapaugcc_proj.species_energies_dict[specieslabel][2]],
                                        label='CBS-aug-cc-45', line=False, marker='x', color='silver')
                    if highest_cardinal > 5:
                        eplot.addseries(0, x_list=[5.5],
                                        y_list=[CCSDTextrap_proj.species_energies_dict[specieslabel][3]],
                                        label='CBS-cc-56', line=False, marker='x', color='orange')
                        if aug_cc_family is True:
                            eplot.addseries(0, x_list=[5.5],
                                            y_list=[CCSDTextrapaugcc_proj.species_energies_dict[specieslabel][3]],
                                            label='CBS-aug-cc-56', line=False, marker='x', color='maroon')
                eplot.addseries(0, x_list=[2.5], y_list=[CCSDTextrapdef2_proj.species_energies_dict[specieslabel][0]],
                                label='CBS-def2-23', line=False, marker='x', color='cyan')
                eplot.addseries(0, x_list=[3.5], y_list=[CCSDTextrapdef2_proj.species_energies_dict[specieslabel][1]],
                                label='CBS-def2-34', line=False, marker='x', color='pink')
            eplot.savefig('{}_Energy'.format(specieslabel))

        # Reaction energy plot
        reactionenergyplot = ASH_plot('{}'.format(reactionlabel), num_subplots=1, x_axislabel="Cardinal",
                                      y_axislabel='Energy ({})'.format(energy_unit), title='{}'.format(reactionlabel))
        if cc_family is True:
            reactionenergyplot.addseries(0, x_list=CCSDT_cc_bases_proj.cardinals,
                                         y_list=CCSDT_cc_bases_proj.reaction_energy_list, label='cc-pVnZ', color='blue')
        if aug_cc_family is True:
            reactionenergyplot.addseries(0, x_list=CCSDT_aug_cc_bases_proj.cardinals,
                                         y_list=CCSDT_aug_cc_bases_proj.reaction_energy_list, label='aug-cc-pVnZ',
                                         color='lightblue')
        if def2_family is True:
            reactionenergyplot.addseries(0, x_list=CCSDT_def2_bases_proj.cardinals,
                                         y_list=CCSDT_def2_bases_proj.reaction_energy_list, label='def2-nVP',
                                         color='red')
        if F12_family is True:
            reactionenergyplot.addseries(0, x_list=CCSDTF12_bases_proj.cardinals,
                                         y_list=CCSDTF12_bases_proj.reaction_energy_list, label='cc-pVnZ-F12',
                                         color='purple')
        if extrapolation is True:
            reactionenergyplot.addseries(0, x_list=[2.5], y_list=[CCSDTextrap_proj.reaction_energy_list[0]],
                                         label='CBS-cc-23', line=False, marker='x', color='gray')
            reactionenergyplot.addseries(0, x_list=[3.5], y_list=[CCSDTextrap_proj.reaction_energy_list[1]],
                                         label='CBS-cc-34', line=False, marker='x', color='green')
            if aug_cc_family is True:
                reactionenergyplot.addseries(0, x_list=[2.5], y_list=[CCSDTextrapaugcc_proj.reaction_energy_list[0]],
                                             label='CBS-aug-cc-23', line=False, marker='x', color='brown')
                reactionenergyplot.addseries(0, x_list=[3.5], y_list=[CCSDTextrapaugcc_proj.reaction_energy_list[1]],
                                             label='CBS-aug-cc-34', line=False, marker='x', color='lightgreen')
            if highest_cardinal > 4:
                reactionenergyplot.addseries(0, x_list=[4.5], y_list=[CCSDTextrap_proj.reaction_energy_list[2]],
                                             label='CBS-cc-45', line=False, marker='x', color='black')
                if aug_cc_family is True:
                    reactionenergyplot.addseries(0, x_list=[4.5],
                                                 y_list=[CCSDTextrapaugcc_proj.reaction_energy_list[2]],
                                                 label='CBS-aug-cc-45', line=False, marker='x', color='silver')
                if highest_cardinal > 5:
                    reactionenergyplot.addseries(0, x_list=[5.5], y_list=[CCSDTextrap_proj.reaction_energy_list[3]],
                                                 label='CBS-cc-56', line=False, marker='x', color='orange')
                    if aug_cc_family is True:
                        reactionenergyplot.addseries(0, x_list=[5.5],
                                                     y_list=[CCSDTextrapaugcc_proj.reaction_energy_list[3]],
                                                     label='CBS-aug-cc-56', line=False, marker='x', color='maroon')
            reactionenergyplot.addseries(0, x_list=[2.5], y_list=[CCSDTextrapdef2_proj.reaction_energy_list[0]],
                                         label='CBS-def2-23', line=False, marker='x', color='cyan')
            reactionenergyplot.addseries(0, x_list=[3.5], y_list=[CCSDTextrapdef2_proj.reaction_energy_list[1]],
                                         label='CBS-def2-34', line=False, marker='x', color='pink')
...
def FormationEnthalpy(TAE, fragments, stoichiometry, RT=False, deltaHF_atoms_dict=None):
    # From ATCT
    deltaHF_atoms_exp_0K = {'H': 51.6333652, 'C': 170.02820267686425, 'N': 112.4710803, 'O': 58.99713193,
                            'F': 18.46510516, 'Si': 107.67925430210326, 'Cl': 28.59010516, 'Br': 28.18283939}
    deltaHF_atoms_exp_298K = {'H': 52.10277247, 'C': 171.33914913957935, 'N': 112.916348, 'O': 59.56716061,
                              'F': 18.96845124, 'Si': 108.71414913957935, 'Cl': 28.97418738, 'Br': 26.73398662}
...
def AutoNonAufbau(fragment=None, theory=None, num_occ_orbs=1, num_virt_orbs=3, spinset=[0], stability_analysis_GS=False,
                  TDDFT=False, epsilon=0.1, maxiter=500,
    ...
    class NonAufbauState:
        def __init__(self, charge, mult, spinset, original_homo_index, occ_index, virt_index, mos_occ_energies,
                     mos_unocc_energies):
            self.charge = charge
            self.mult = mult
        ...
        for v in range(0, num_virt_orbs):
            ...
            for s in spinset:
                if s == 0:
                    ...
                    cfg = NonAufbauState(fragment.charge, fragment.mult, s, homo_number_alpha, occ_index, virt_index,
                                         mo_dict["occ_alpha"], mo_dict["unocc_alpha"])
                elif s == 1:
                    occ_index = homo_number_beta - o
                    virt_index = lumo_number_beta + v
                    cfg = NonAufbauState(fragment.charge, fragment.mult, s, homo_number_beta, occ_index, virt_index,
                                         mo_dict["occ_beta"], mo_dict["unocc_beta"])
    ...
    for state in calculated_states:
        ...
        print("=" * 90)
        print(
            f"Now running excited state SCF no. {state_index} with multiplicity: {cfg.mult} and spinset {cfg.spinset}")
        print("=" * 90)
        ...
        if TDDFT is True:
            ...
            print(
                f"Will rotate occupied orbitals: {orbpair_occ_A} and virtual orbital: {orbpair_unocc_A} in spin manifold {spinvar_A}")
            ...
            if multiple_pairs is True:
                ...
                print(
                    f"Will rotate occupied orbitals: {orbpair_occ_B} and virtual orbital: {orbpair_unocc_B} in spin manifold {spinvar_B}")
    ...
    shifterr 0.0005
        ...
        shutil.copy(theory.filename + '.out',
                    theory.filename + f'ES_SCF{state_index}_mult{cfg.mult}_spinset{cfg.spinset}.out')
        shutil.copy(theory.filename + '.gbw',
                    theory.filename + f'ES_SCF{state_index}_mult{cfg.mult}_spinset{cfg.spinset}.gbw')
    ...
    for state in calculated_states:
        ...
        print(
            f"Excited-state SCF orbital rotation: Occ:{states_dict[state_index][1]} Virt:{states_dict[state_index][2]}")
        print(f"Rotation in spin manifold: ", states_dict[state_index][3])
        print(
            f"Excited-state SCF HOMO-LUMO gap: {states_dict[state_index][4]} Eh ({states_dict[state_index][4] * 27.211399}) eV: ")
...
# TODO: Future, wr
def ExcitedStateSCFOptimizer(theory=None, fragment=None, autononaufbaudict=None, state=1, charge=None, mult=None,
                             maxiter=500, Freq=False, extrashift=0.0):
    ...
    if Freq:
        ...
        result_freq = ash.NumFreq(fragment=fragment, theory=theory, npoint=2, runmode='serial', charge=charge,
                                  mult=mult)
