...
from openmmqmmm.constants import hartokcal
from openmmqmmm.dictionaries_lists import eldict
from openmmqmmm.functions.functions_general import ashexit, check_program_location, isodd, print_line_with_mainheader, pygrep, \
    print_pretty_table
from openmmqmmm.interfaces.interface_ORCA import ORCATheory, run_orca_plot, make_molden_file_ORCA
from openmmqmmm.interfaces.interface_multiwfn import multiwfn_run
from openmmqmmm.modules.module_coords import nucchargelist, elematomnumbers
...
cnvkdiis false
        ...
        # Element coordinates
        Elfrag = openmmqmmm.Fragment(elems=[el], coords=[[0.0, 0.0, 0.0]])
        print("Elfrag dict ", Elfrag.__dict__)
        openmmqmmm.Singlepoint(theory=ORCASPcalculation, fragment=Elfrag, charge=0, mult=spindictionary[el])
...
def DDEC_to_LJparameters(elems, molmoms, voldict, scale_polarH=False):
    ...
    for count, el in enumerate(elems):
        ...
        atmnumber = openmmqmmm.modules.module_coords.elematomnumbers[el.lower()]
        print("atmnumber:", atmnumber)
        Radii_vdw_free.append(openmmqmmm.dictionaries_lists.elems_C6_polz[atmnumber].Rvdw_ang)
        print("Radii_vdw_free:", Radii_vdw_free)
        volratio = molmoms[count] / voldict[el]
        print("volratio:", volratio)
        C6inkcal = openmmqmmm.constants.harkcal * (
                openmmqmmm.dictionaries_lists.elems_C6_polz[atmnumber].C6 ** (1 / 6) * openmmqmmm.constants.bohr2ang) ** 6
        print("C6inkcal:", C6inkcal)
        B_i = C6inkcal * (volratio ** 2)
        print("B_i:", B_i)
        Raim_i = volratio ** (1 / 3) * openmmqmmm.dictionaries_lists.elems_C6_polz[atmnumber].Rvdw_ang
        print("Raim_i:", Raim_i)
...
def num_core_electrons(elems):
    sum = 0
    # formula_list = openmmqmmm.modules.module_coords.molformulatolist(fragment.formula)
    for i in elems:
        cels = openmmqmmm.dictionaries_lists.atom_core_electrons[i]
        sum += cels
    return sum
...
def Jcoupling_Yamaguchi(HSenergy, BSenergy, HS_S2, BS_S2):
    ...
    J_kcal = J * openmmqmmm.constants.harkcal
    J_cm = J * openmmqmmm.constants.hartocm
...
def difference_density_ORCA(fragment_A=None, fragment_B=None, theory_A=None, theory_B=None, griddensity=80,
    ...
    result_calc1 = openmmqmmm.Singlepoint(theory=theory_A, fragment=fragment_A)
    ...
    result_calc2 = openmmqmmm.Singlepoint(theory=theory_B, fragment=fragment_B)
    # Run orca_plot to request electron density creation from ORCA gbw file
...
def NOCV_density_ORCA(fragment_AB=None, fragment_A=None, fragment_B=None, theory=None, griddensity=80,
    ...
    print("-" * 120)
    result_promol = openmmqmmm.Singlepoint(theory=promolecule_AB_orthog, fragment=fragment_AB)
...
"""
    ...
    print("-" * 120)
    result_calcAB = openmmqmmm.Singlepoint(theory=calc_AB, fragment=fragment_AB)
    if make_cube_files is True:
        # Run orca_plot to request electron density creation from ORCA gbw file
    ...
    result_calcAB_noiter = openmmqmmm.Singlepoint(theory=calc_AB, fragment=fragment_AB)
...
def NOCV_Multiwfn(fragment_AB=None, fragment_A=None, fragment_B=None, theory=None, gridlevel=2, openshell=False,
    ...
    if isinstance(theory, ORCATheory) is not True:
        ...
        ashexit()
    # A
    result_calcA = openmmqmmm.Singlepoint(theory=theory, fragment=fragment_A)
    make_molden_file_ORCA(theory.filename + '.gbw')  # TODO: Generalize
    os.rename("orca.molden.input", "A.molden.input")
    theory.cleanup()

    # B
    result_calcB = openmmqmmm.Singlepoint(theory=theory, fragment=fragment_B)
    ...
    result_calcAB = openmmqmmm.Singlepoint(theory=theory, fragment=fragment_AB)
...
ScalHFX {percentages[0]}
    ...
    result_1 = openmmqmmm.Singlepoint(theory=theory_1, fragment=fragment)
    blocks_2 = f"""
%method
ScalHFX {percentages[1]}
    ...
    result_2 = openmmqmmm.Singlepoint(theory=theory_2, fragment=fragment)
    ...
    result_1b = openmmqmmm.Singlepoint(theory=theory_1b, fragment=fragment)

    # Define metrics
    delta_E_D = (result_1b.energy - result_1.energy) * openmmqmmm.constants.harkcal
    delta_E = (result_2.energy - result_1.energy) * openmmqmmm.constants.harkcal
    delta_E_F = (result_2.energy - result_1b.energy) * openmmqmmm.constants.harkcal
...
def boltzmann_populations(energies, temperature=298.15):
    ...
    beta = 1 / (openmmqmmm.constants.R_gasconst_kcalK * temperature)

    rel_energies = np.array([en - min(energies) for en in energies]) * openmmqmmm.constants.hartokcal
